...
CREATE TABLE IF NOT EXISTS zone_runs (
    ...
    -- ADR-018 decision recording (§3.6 S5). Nullable: rows written before S5, or
    -- a cycle whose decision the writer did not hold (AppDaemon restart
    -- mid-cycle), keep NULL. Added to already-created databases by the
    -- bootstrap's column migration (db_schema_init.py COLUMN_MIGRATIONS) --
    -- keep the two definitions identical.
    season                TEXT                         -- zone season at run time
                            CHECK (season IS NULL OR
                                   season IN ('spring', 'summer', 'fall', 'winter')),
    decision_criteria     TEXT,                        -- JSON: the applied decision
                                                       -- (compute_zone_programs criteria)
...
-- 2026-09-23  zone_runs + `season` (CHECK spring/summer/fall/winter, nullable) and
--             `decision_criteria` (JSON TEXT) -- ADR-018 decision recording,
--             §3.6 S5. Existing databases get them via db_schema_init.py's
--             idempotent ALTER TABLE ADD COLUMN migration (no rebuild needed:
--             ADD COLUMN supports a nullable column with a CHECK).
-- =============================================================================
