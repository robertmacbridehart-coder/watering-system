...
**Gate 7.1 LOCKED 2026-08-21** (visual tokens), **Gate 7.2 Information Architecture COMPLETE
2026-08-28** (all six views specced + entity-mapped in `docs/ui_design.md` v0.7.3). Next: **Gate 7.3
High-Fidelity Static Mockup**.
...
### 1.4 Soil Moisture Sensors

**Wireless (plan of record):** ✅ **4× Ecowitt WH52 installed 2026-08-29** on a GW3000
gateway. They are live in HA as `sensor.gw3000a_soil_{moisture,temperature,electrical_conductivity,battery}_N`.
- **Sensor layer:** `ecowitt.yaml` (§3.6 S1).
- **Calibration:** per-bed gateway calibration (air-dry / saturated sample) is **pending**
  (`soil_calibration_log.md` §4).

**Wired RS-485 SEN0600 (items below): ⏸ DEFERRED indefinitely** (ADR-021 Amendment A,
2026-09-23). Revisit after a season of recorded decisions.
- [ ] ⏸ Change sensor addresses to 0x05, 0x06, 0x07
- [ ] ⏸ Add Modbus controller entries
- [ ] ⏸ Test moisture/temperature readings
- [ ] ⏸ Add sensor reading scheduler (R10 pulse-poll; ADR-021)

**Blocker Notes:** _SEN0600 deferred by decision, not blocked._
...
4. **V2 Zone Configuration (68 helpers):**
   ...
   - *2026-09-23: the 48 `*_moisture_min` above are **RETIRING** (ADR-021 Amendment A; removed
     at §3.6 S2-rework). Fert eligibility re-points to the per-zone guard helpers:
     `zone_N_moisture_dry_bump < moisture < zone_N_moisture_wet_veto` (architecture §2.2).*
...
- Soil moisture band gate (`dry_bump < moisture < wet_veto`, re-pointed 2026-09-23), rain fallback
...
- [x] ✅ **Decision-criteria recording on `zone_runs` (ADR-018).** Built as §3.6 **S5**
      (2026-09-23): `season` + `decision_criteria` (the applied `compute_zone_programs`
      criteria, JSON) via Event 7 + the `db_writer` buffer; `watering_cycles` weather columns
      now record the decision's inputs. Live check: Test 10.10.
- [x] ✅ **Extract the §6.2 decision into one shared, side-effect-free routine (ADR-018).**
      Built as `script.compute_zone_programs` (§3.6 **S2**, 2026-08-30/31), reworked to
      ADR-021 Amendment A. `state_window_check` applies it since §3.6 **S4** (2026-09-23).
...
**Status (2026-09-23):** ✅ **S1–S5 BUILT AND LIVE** (parked until go-live).
- **S1–S5 are done and live** on the Green (S3 validated end to end; S4 live-verified;
  S5 Test 10.10 passed, 2026-09-23). Left: a real cycle's decision row (needs the ESP32
  online), the effectiveness monitor after the drip install, and the deferred tuning
  items.
- Build steps are named **S1–S5** (renamed 2026-09-23 from "Phase 1–5",
  which collided with the roadmap's own phases).
- **ADR-021 Amendment A (ACCEPTED 2026-09-23)** makes the shared routine compute a
  **weather-primary tree + moisture guard** (wet veto, optional dry bump), not a moisture
  ladder.
- **Recording (this section)** is the evidence base: every input is stored, so a
  moisture-led policy can be re-evaluated offline before moisture gets more authority.
- **Constraint:** the system stays parked (`manual_override`) as the actuation gate until
  go-live, so everything before S4 is record-only.

*History:* ADR-018 is the single decision-recording mechanism (ADR-021's standalone
`decisions` table was retired, 2026-08-25). Its two deferred items were resolved by ADR-021:
the de-lagged temperature, and RestoreEntity helpers.
...
Build steps (§3.6 **S1–S5**, in order):

**S1 — Foundations** ✅ DONE (committed `a2b1895` + `352caa9`; pushed 2026-09-23).
The **AppDaemon half was not live until 2026-09-23.** The S3 live fire exposed two deploy
gaps; HA-side checks had shown only the HA half working.
- **(a)** `ModuleNotFoundError` for both weather apps. A new app folder isn't on
  AppDaemon's startup-built import paths; fixed by one add-on restart.
- **(b)** `docs/weather_schema.sql` was missing from the public-mirror whitelist
  (`.github/sanitize.py`), so the schema never deployed and the DB was never created.
  Whitelisted 2026-09-23.
- [x] ✅ `docs/weather_schema.sql`: two tables, FK + CHECK + indexes, UTC TEXT timestamps.
      Validated on a scratch SQLite DB.
- [x] ✅ AppDaemon bootstrap (`watering_weather/weather_db_schema_init.py`, idempotent). The
      schema deploy is wired into `pull_public_repo.sh`.
- [x] ✅ `weather_logger.py`: a dumb sink for Event 6 `watering_weather_snapshot`. It writes one
      snapshot + N decisions in a single transaction. `tests/test_weather_logger.py` has 22
      assertions, all passing.
- [x] ✅ Event 6 payload contract (architecture §13.3.1): snapshot columns flat at top level +
      `zone_decisions[]`.
- [x] ✅ Forecast inputs (`dwd_brightsky.yaml`): `forecast_pop_today` / `forecast_rain_today`
      (rest of today) and `forecast_temp_high_today` / `_tomorrow` (window-aware), all off one
      widened fetch.
- [x] ✅ Ecowitt sensor layer (`ecowitt.yaml`): zone tags, fresh-only per-zone moisture
      averages, the rain-source abstraction, and the 3 h OR-of-3 WH52 liveness gate.

**Deploy checkpoint** ✅ DONE 2026-09-23
- [x] ✅ Repo pull → restart (22:20 UTC), then a second pull + template reload for the fix
      `63e6381`.
- [x] ✅ `moisture_wireless_4_zone` = Blueberries.
- [x] ✅ `calculate_zone_runtime` → `resolve_program_multiplier` smoke test passed (operator,
      Dev Tools).
- [x] ✅ New entities verified live (read-only): zone 1 19.5 % (2 sensors), zone 2 26 %,
      zones 3/4 unknown; heartbeats correct (CH1 + WH40 off); forecast + rain layers live.
      Two bugs were found and fixed en route (`63e6381`).

**S2 — Shared decision routine** ✅ REWORKED to Amendment A **and DEPLOYED + VERIFIED LIVE** (2026-09-23)
- [x] ✅ **Live checks after pull + full restart:**
  - all 80 weather thresholds kept their values (script compare vs git: 0 mismatches);
  - 13 guard helpers present at 0 / off; 8 wet/dry sensors off;
  - `compute_zone_programs` loaded under its new alias;
  - zone 1 19.5 % / zone 2 26 %; the CH4 tag survived the restart;
  - `wh52_soil_1_online` off straight after the restart (heartbeat fix confirmed);
  - park on.
  - The operator removed the orphaned registry entries (the 48 `*_moisture_min` + 3 older).
- [x] ✅ `script.resolve_program_multiplier`: the single program→multiplier source.
      `calculate_zone_runtime` delegates to it, behaviour-preserving (56 combos + 9 spot
      checks equal; live Dev-Tools smoke test passed 2026-09-23).
- [x] ✅ `script.compute_zone_programs` **reworked**:
  - weather tree → dry bump → forecast downgrade → wet veto (final);
  - the live ladder is dropped;
  - `decision_criteria` records the branch, `wp_weather`, the modifiers, `skip_reason`, the
    guard state + thresholds + hysteresis, and the weather thresholds (offline replay);
  - side-effect-free; returns the Event-6 payload.
- [x] ✅ New `packages/watering_state/moisture_guard.yaml`:
  - per-zone `zone_N_moisture_wet_veto` / `_dry_bump` / `_dry_bump_enabled` + the shared
    `moisture_guard_hysteresis`, all RestoreEntity, with **0 = disabled** as the first-boot
    value;
  - hysteresis-held `binary_sensor.zone_N_moisture_wet` / `_dry` (they read their own
    previous state via `this.state`).
- [x] ✅ Removed the 48 `zone_N_{season}_{off,light,normal}_moisture_min` from `zone_helpers.yaml`.
- [x] ✅ Converted the 80 **weather** threshold helpers to RestoreEntity (dropped `initial:`;
      formerly "Phase 2b"). **After the next restart, verify the values survived** (e.g.
      `zone_1_spring_rain_off_mm` = 20).
- [x] ✅ Offline tests promoted to **`tests/ha_templates/`** (`run_all.py`):
  - decision tree 31, moisture guard 24, Ecowitt templates 23, multiplier 10,
    temp horizon 7 — all passing.
  - Requires PyYAML + jinja2; not deployed.
- [ ] 📋 **Deferred to after S3 data:** hysteresis on the *rain* gates (Amendment A Decision 6).

**S3 — Always-on recording** 🔧 BUILT 2026-09-23 (**recording starts at deploy**, while parked)
- [x] ✅ Automation `watering_weather_snapshot_recorder`
      (`packages/watering_weather/weather_recording.yaml`):
  - triggers on `morning_window_start` / `evening_window_start` with **no conditions**,
    so it fires in every state;
  - calls `compute_zone_programs(window = trigger id)` and fires Event 6. The decision
    fields come from the response; the observational columns and `raw` (per-probe soil,
    dew point, wind as reported) are added.
  - Manual "Run actions" records the window matching the time of day.
- [x] ✅ **S2 fix found here:** the routine now takes the booster window from its `window`
      field, not `input_select.active_watering_window`. That helper is only set by the
      scheduler, so it goes stale while parked.
- [x] ✅ The criteria now also carry the context flags `window_enabled` / `manual_override` /
      `winterized`.
- [x] ✅ Offline end-to-end test `tests/ha_templates/test_s3_event_chain.py` (18 checks):
      zbase → zones → result → recorder `event_data` → the **real** `WeatherLogger` →
      SQLite from `weather_schema.sql`. The suite is now 6 files / 117 checks, all passing.
- [x] ✅ **Live-validated END-TO-END 2026-09-23** (after the whitelist fix + an AppDaemon
      add-on restart; the log showed "watering_weather schema verified/created (2 tables
      present)").
  - Manual fire at 01:20:54 Berlin wrote **`snapshot_id` 1 (a TEST row: manual, labelled
    morning)** + 4 decisions. They matched the prediction exactly:
    - `weather_available` 1, forecast high 18.0;
    - all zones `warm` → `normal` → `off` (zones disabled);
    - moisture 19.5 / 2 · 26.0 / 1 · NULL / 0 · NULL / 0.
  - `temp_c` was NULL only because BrightSky current-weather hadn't fetched 2 min after the
    restart; the 01:13 payload had 7.6.
  - Earlier fires were lost to the two deploy gaps and the AppDaemon restart window (no
    rows).
- [x] ✅ `observed_at` switched to **UTC** (`utcnow()`) per the schema; `snapshot_id` 1
      predates the fix (`+02:00`).
- [x] ✅ First **scheduled** rows confirmed: `snapshot_id` 2 (06:00 Berlin, pre-fix `+02:00`)
      and **`snapshot_id` 3 `2026-09-23T16:00:00.095113+00:00`**, the first UTC row.
- [ ] 📋 Traceback for the `bright_sky_warm_slow_rest_sensors_on_start` "Unexpected error" at
      startup. Its fetches still succeed; it may predate S1.
- [ ] 📋 Confirm `/homeassistant/watering_weather.db` is inside the HA backup.
- [ ] 📋 Verify the BrightSky wind unit (sensor labelled m/s; the default `dwd` units are
      probably km/h). Until then `wind_speed_ms` stays NULL and the value goes to `raw`.

**S4 — Apply** ✅ BUILT + DEPLOYED + LIVE-VERIFIED (parked) 2026-09-23 (**SAFETY-ADJACENT**; park-gated; behaviour stays close to
the previously deployed weather tree)
- [x] ✅ `script.state_window_check` rewired (`state_scripts.yaml`; checklist approved
      2026-09-23):
  - calls `compute_zone_programs(window = input_select.active_watering_window)` and applies
    the programs **by `zone_id`**; its inline weather tree + cadence copy are deleted
    (one decision source);
  - **fail-safe:** the routine not loaded (checked first, since `continue_on_error` does
    not catch `ServiceNotFound`), a call error, an empty response, ≠ 4 zones, a duplicate
    zone or an unknown program → every zone `off` + an `error` log → idle;
  - `continue_on_error` on the call = a **documented exception to the ADR-015
    no-`continue_on_error` rule** (the routine
    actuates nothing; an abort would strand the machine in `window_check`);
  - the D-A warning is keyed on the routine's `weather_available`; a new per-window
    `info` breadcrumb `Programs (<window>): Z1 … · Z2 off (<reason>) …`;
  - A1 guard-the-advance and the idle / preflight transitions are unchanged.
- [x] ✅ Offline test `tests/ha_templates/test_s4_window_check.py` (35 checks): real
      routine output → extraction; 7 defect shapes → all off; the missing-routine guard;
      structure (top-level call, no inline logic left, A1 intact). Suite: 7 files / 154
      checks, all passing.
- [x] ✅ **Live pass (parked) PASSED 2026-09-23** (Test 4.24):
  - all-off case, then the zone-1 `normal` case, matching the prediction and proving the
    response round-trip;
  - A1 held; nothing actuated.
  - Breadcrumb text verified in `system_events` (rows 222/223).
  - **Un-parked full path verified** (operator): scheduler manual branch → dispatcher →
    window_check → all off → idle in ~180 ms; preflight never reached.
- [ ] 📋 Full end-to-end cycle through the rewired check: deferred until the ESP32 is online
      (with Test 4.22's DB-tag case).
- [ ] 📋 Optional follow-up: a stuck-state watchdog (a non-idle operational state held too
      long → alert). S4's fail-safe covers a missing routine and caught call errors; an
      exception `continue_on_error` does not catch could still strand `window_check`.

**S5 — Ops recording + export + docs** ✅ BUILT + DEPLOYED + LIVE-VERIFIED 2026-09-23 (`abb1c62`) (checklist approved; operator chose
the writer buffer + the Event 1 alignment)
- [x] ✅ **`zone_runs` `season` + `decision_criteria`** (nullable; `db_schema.sql`).
  - **Live table:** new idempotent column migration in `db_schema_init.py`
    (`COLUMN_MIGRATIONS` → `ALTER TABLE ADD COLUMN`). `CREATE TABLE IF NOT EXISTS` alone would
    never reach the existing table.
  - **`season`:** Event 3 carries `input_select.zone_N_season`.
  - **`decision_criteria`:** `state_window_check` fires **Event 7
    `watering_decision_applied`** (the applied `zone_decisions`) just before preflight. The
    `db_writer` buffers it, attaches it at Event 1 (≤ 10 min old), and writes each zone's
    criteria as JSON at Event 3.
  - **Degradation:** missing, stale or restart-lost → NULL. A bad season → NULL. The run row
    is always written.
- [x] ✅ **Event 1 alignment:** `watering_cycles` rain = `sensor.rain_24h` / `rain_72h` (the
      decision's WH40-or-DWD source), `temp_high_c` = the window's forecast high (was
      BrightSky rain + yesterday's high; older rows keep the old sources).
- [x] ✅ **Export:** `db_export.py` also writes `weather_snapshots_YYYY.csv` +
      `zone_decisions_YYYY.csv` from `watering_weather.db`, opened **read-only**.
  - A missing weather DB skips only those two files and marks the audit row `warning`.
  - No `db_automations.yaml` change: the same winterize trigger drives both exports.
- [x] ✅ **Tests** (all passing):
  - `test_db_schema_init.py` (NEW, 11);
  - `test_db_writer.py` (+15 → 39);
  - `test_db_export.py` (+11 → 22);
  - `tests/ha_templates/test_s5_decision_recording.py` (NEW, 14: real routine → Event 7 →
    REAL writer → `zone_runs`).
  - Template suite: 8 files / 168 checks.
- [x] ✅ **Docs:** architecture v1.11.0 §13, Test 10.10, programming-notes, START_HERE,
      `db_setup_guide.md`.
- [x] ✅ **Live checks (Test 10.10) PASSED 2026-09-23:**
  - the migration added both columns in place (8 rows kept, NULL) and was idempotent on
    the add-on restart;
  - the Event 7 listener is live;
  - the export wrote 3 snapshots / 12 decisions, with an `info` audit row (230).
- [ ] 📋 **Deferred until the ESP32 is online:** a real cycle's `zone_runs` row with criteria
      (pair with Test 4.22 DB-tag).

**Later (post drip install / go-live):**
- [ ] 📋 Effectiveness monitor (Amendment A Decision 4): post-run moisture rise, dry-down rate,
      and a "watered but no rise" fault signal. Design first.
...
**Dependencies:**
- ADR-018 ✅ and ADR-021 + Amendment A ✅ (accepted).
- ADR-011 (DB pattern), ADR-013 (event → writer).
- **The per-bed gateway calibration** (`soil_calibration_log.md` §4) must be done before the
  guard helper **values** are set. It does not block S2–S3 code: the guard stays neutral
...
> **REWORK — ADR-021 as amended by Amendment A (ACCEPTED 2026-09-23; architecture §3.2, v1.10.0).**
>
> - **Design:** the tree below stays **primary**, upgraded with the current-rain skip, the
>   de-lagged **window-aware** forecast high (replacing `temp_avg_high_3day`), the capped
>   forecast-rain downgrade, and a **soil-moisture guard** (optional dry bump + a final,
>   hysteresis-held wet veto).
> - **Reversed:** ADR-021's original moisture-primary ladder, on the basis of 3.5 weeks of
>   WH52 data.
> - **Where it lives:** the shared `compute_zone_programs` (§3.6 S2-rework), applied by
>   `state_window_check` since §3.6 **S4** (2026-09-23). The tree below is superseded
>   (kept as the Phase-4 build record).
> - **Recording** is ADR-018 (§3.6 S3). Test 4.24 (test_scenarios) is being respecified to
>   the amended tree.
...
> **Current position: Gate 7.0 COMPLETE (2026-08-20). Gate 7.1 LOCKED (2026-08-21). Gate 7.2
> Information Architecture COMPLETE (2026-08-28, user sign-off)** — all six views have detailed
> entity-mapped specs in `docs/ui_design.md` v0.7.3, wireframes approved, PIP mock in hand.
> Next: **Gate 7.3 — High-Fidelity Static Mockup** (render the hero views as HTML Artifacts in the
> locked glass style; user approves the look before any 7.4 YAML).
...
### 7.2 Information Architecture — ✅ COMPLETE (2026-08-28, user sign-off)
- [x] ✅ Dashboard(s) + views defined; card inventory per view; navigation model
- [x] ✅ Each card mapped to its entities and its interaction type (read / edit / action)
- [x] ✅ Wireframe-level layout (structure only, no styling) reviewed and approved

**Deliverable:** `docs/ui_design.md` v0.7.3 — detailed per-view specs for all six surfaces: Overview
(§2.6, LOCKED), Zones zone-card (§2.2.1, LOCKED), Zone Advanced (§2.2.2), System Advanced (§2.3.1),
History/Stats (§2.4.1), Diagnostics/Health incl. the interactive Comms Check (§2.5.1); shared
Advanced shell (§2.3.0); see/do split (§3). Every card mapped to verified entity IDs. The PIP
wireframe/mock (user + ChatGPT: `WS PIP.svg` + an HTML mock) satisfies the wireframe-approval item;
the responsive production direction is in §6.11. Background LOCKED (`ui_background.png`). Remaining
Phase-7 items are build-gated (§7.4 deps) or hardware/ADR-blocked — tracked in `ui_design.md` §7.

**How:** map the content inventory onto views as wireframes (ASCII in `ui_design.md`); confirm entity
coverage via read-only hass-mcp / `entity_reference.md`; get approval before any styling.
...
- **2026-09-23:** **§3.6 S5 deployed + live-verified** (Test 10.10). The first UTC scheduled
  weather row is confirmed (`snapshot_id` 3). §3.6 S1–S5 are complete; a real-cycle decision row
  waits for the ESP32.
- **2026-09-23:** **§3.6 S5 built.**
  - `zone_runs` gains `season` + `decision_criteria`, with a live-DB column migration.
  - Event 7 → `db_writer` decision buffer.
  - Event 1 weather columns now record the decision's inputs.
  - The weather tables are added to the seasonal export (read-only).
  - New/extended tests; Test 10.10 added for the live checks. The §3.5 decision-criteria item
    is ticked.
- **2026-09-23:** **§3.6 S4 deployed + live-verified:** the parked all-off and zone-1 `normal`
  cases matched prediction; the breadcrumb was verified; the un-parked full path went to idle
  (Test 4.24).
- **2026-09-23:** **§3.6 S4 built — `state_window_check` applies `compute_zone_programs`.**
  One decision source for recording and actuation; fail-safe all-off → idle on a routine
  failure (a documented exception to the no-`continue_on_error` rule); per-window breadcrumb; new offline test
  (35 checks). §3.5 / §6.2 references updated. Live Dev-Tools pass pending the pull.
- **2026-09-23:** **ADR-021 Amendment A cascade + §3.6 restructured into S1–S5.**
  - **§3.6 rewritten** from the stale "design only, no code" to the real state:
    - **S1** foundations ✅ (`a2b1895`, `352caa9`);
    - a **deploy checkpoint** (pull, CH4 tag, `calculate_zone_runtime` smoke test);
    - **S2** routine built, **rework to Amendment A** pending (retire the 48 moisture ladder
      helpers, add guard helpers, weather helpers → RestoreEntity, promote the offline
      tests);
    - **S3** always-on recording (next);
    - **S4** `state_window_check` rewire (safety-adjacent, target before spring 2027);
    - **S5** `zone_runs` columns + weather export + docs;
    - effectiveness monitor after the drip install.
  - **Renamed** the weather-DB build "Phase 1–5" → **S1–S5** to stop colliding with the
    roadmap phases.
  - **§3.5** routine-extraction item ticked (built as S2).
  - **§6.2** banner updated to the amended weather-primary + moisture-guard tree.
  - **§1.4:** WH52 installed (plan of record); **SEN0600 deferred indefinitely**.
- **2026-08-28:** **Gate 7.2 (Information Architecture) marked ✅ COMPLETE** (user sign-off). All six
  dashboard views now have detailed, entity-mapped specs in `docs/ui_design.md` v0.7.4 (Overview,
  Zones zone-card, Zone Advanced, System Advanced, History/Stats, Diagnostics/Health incl. the
  interactive Comms Check); wireframes approved; PIP mock in hand (user + ChatGPT: `WS PIP.svg` +
  HTML). §7.2 checkboxes ticked; current-position note updated. **Next: Gate 7.3 (High-Fidelity
  Static Mockup).** Remaining Phase-7 work is build-gated (7.4 deps) or hardware/ADR-blocked — see
  `ui_design.md` §7.
