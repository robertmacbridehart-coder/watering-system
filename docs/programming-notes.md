...
- **Documented exception (2026-09-23, §3.6 S4):** `state_window_check`'s call to the
  side-effect-free `script.compute_zone_programs` carries `continue_on_error`. The rule
  protects *actuating* subscripts whose failure must reach a per-error automation. The
  routine actuates nothing, and an aborted `window_check` would strand the machine (no
  stuck-state watchdog; the scheduler only starts from idle), silently stopping all
  watering.
  - A failed call instead fails safe: all zones `off`, an `error` log, back to idle.
  - A missing routine is checked **before** the call, because `continue_on_error` does
    not catch `ServiceNotFound` (see "`continue_on_error` Scope" above).
  - The call sits at the **top level**, not inside an `if`, so its `response_variable`
    is in scope for the later steps. Nested-block variable scoping is not relied on.
...
**Amendment 2026-08-26 (Phase 7.2 dashboard — daily rain/temp history, to resolve AT BUILD-OUT).**
The Overview **Weather card** wants a 7-day history: a **per-calendar-day rain total (mm/day)** and
**daily high temp (°C)** (rendered as a rain bar chart — daily total under the x-axis, high-temp text
above each bar). The current ADR-018 design does **not** capture this cleanly: `weather_snapshots`
holds a **twice-daily** (morning/evening window) sample of the *rolling* `rain_24h` value — two
rolling-window samples per day is **not** a clean calendar-day total. **Resolve during the ADR-018
build-out** (do NOT build now — deferred with the rest of ADR-018): add a **daily-granularity
capture** — a once-daily observation row for the *completed prior day* with first-class
`rain_total_mm` / `temp_high_c` (`temp_low_c` cheap to add) — and/or source the dashboard history via
a **REST/history sensor** pulling day-totals from HA at render (the user's provisional lean). Decide
the exact mechanism then. **Decoupling note:** this daily capture depends only on **BrightSky (already
live)**, NOT on the held moisture/weather-station hardware, so if the dashboard needs it before the
rest of ADR-018, this slice can be pulled forward independently (a BrightSky backfill would populate
history retroactively). Until it lands, the Weather card shows **today's forecast only** (live
`brightsky_forecast_*`); the weekly-history section is **⏸ HELD**.
...
**Date:** 2026-08-22
**Status:** Accepted (Phase 7); **committed (`03554c9`) + DEPLOYED to the Green 2026-08-25; VALIDATED
LIVE 2026-08-26 — Test 4.22 passed 16/17** (all program-selection cases + both `calculate_zone_runtime`
multipliers 1.0×/1.5× + the SQL sensor-query filters). The one outstanding case, **DB-tag** (a real
booster run persisting `heavy`+`0.5` via the AppDaemon Event-3 writer), is **deferred until the ESP32
is online** (currently offline while valves/pump are unwired). Supersedes ADR-007 and ADR-009; closes
ADR-015 D-C.
...
**Date:** 2026-08-25
**Status:** **ACCEPTED** (Phase 7, 2026-08-25); design + doc cascade committed (`31abe3c`) and on the
Green. **NO code written — implementation and deploy remain HELD** until the soil-moisture hardware
(4× WH52 + 3× SEN0600 + WH40H + Ecowitt gateway) is installed and calibrated. The deployed system
still runs the ADR-020 weather-only tree (now ADR-021's fallback). One open tuning detail: hysteresis
band sizes (set against ground-truth).
**⚠ AMENDED by Amendment A (ACCEPTED 2026-09-23)** (see below): weather-primary trigger,
moisture as guard + monitor, per-bed gateway calibration, SEN0600 deferred indefinitely.
It reverses Decision 1 on the basis of 3.5 weeks of WH52 data. The hardware "HELD" note above
is historical: the 4× WH52 + GW3000 were installed 2026-08-29.
...
#### ADR-021 Amendment A: Weather-Primary Trigger, Moisture as Guard + Monitor (2026-09-23)

**Status:** **ACCEPTED** (operator, 2026-09-23), together with deferring SEN0600
indefinitely and renaming the weather-DB build steps to §3.6 **S1–S5** (impl_roadmap).
- **Amends** ADR-021 Decision 1 (moisture primary) and its rejected alternative *"moisture as
  override, weather primary"*, which is now adopted.
- **Retains** ADR-021's current-rain skip, de-lagged temperature, capped forecast-rain
  downgrade, graceful degradation, sensor → zone mapping, rain-source abstraction and
  ADR-018 recording.
- **Docs cascaded 2026-09-23:** architecture v1.10.0 §1.2/§2.2/§3.2/§4.2/§4.2.6/§9.1,
  entity_reference, impl_roadmap §3.6/§6.2, test_scenarios Test 4.24, ui_design §7,
  START_HERE.
- **Code:** reworking `script.compute_zone_programs` is §3.6 **S2-rework** (next build step).

**Context: what the ground truth showed.** 3.5 weeks of live WH52 data
(`soil_calibration_log.md` §6, 2026-08-29 → 09-22) undercut the assumptions behind a moisture
ladder:
- **The band is narrow.** Lowest observed → field capacity is only ~10–13 pts
  (zone 1 ≈ 16–27 %, zone 2 ≈ 21–34 %, factory scale). Readings are whole percent, dither
  ±1, and shift ±1–2 overnight with soil temperature. The four ladder rungs would each be
  ~2.5 pts wide, i.e. the size of the noise.
- **Spots differ more than the rungs.** The three zone-1 probes read 16.7 / 19.0 / 12.8 on
  the same day. With drip, a probe will mostly report how close it sits to an emitter.
- **The dry anchor is unknown.** The lowest reading was not a floor and is not a known
  plant-stress point, so a "water when below X" trigger would rest on a guess.
- **Hand-waterings barely registered.** They left only +2–4 pts by the next morning, so
  moisture is a weak "how much" signal.
- **Deployed thresholds are off-scale.** Off/light/normal = 75/55/40 % (spring) is ~2× this
  soil's range: the ladder as specified would choose `heavy` almost every day and never
  wet-skip.
- **Where moisture is strong:** wetting events (rain, watering) jump +10–40 pts, well
  clear of the noise, and the rate of change is clean. The first genuine RF dropout
  (CH4, 09-13, ~5 h) also shows that sensor health has to gate any moisture use.

**Decision (amends ADR-021 Decisions 1, 3 and 6):**

1. **Weather + cadence is the PRIMARY trigger and sets intensity.** `wp` comes from the
   §3.2 weather tree, keeping ADR-021's upgrades:
   - current-rain hard skip (2);
   - de-lagged temperature (3), now **window-aware**: the morning window uses today's
     forecast high, the evening window tomorrow's (`sensor.forecast_temp_high_{today,tomorrow}`,
     2026-08-31);
   - the capped/floored forecast-rain downgrade (4);
   - graceful degradation (5).

   The ADR-020 cadence/booster layer is unchanged.
2. **Moisture is a GUARD, not a ladder.**
   - **Wet veto:** zone moisture ≥ `wet_veto` → `off` (`skip_reason: moisture_wet`). This is
     the local-rain catch for root cause (c).
   - **Dry bump (optional, disabled by default):** zone moisture ≤ `dry_bump` → +1 step,
     capped at `heavy`. Enable per zone only once field data justify it.
   - Both use a **hysteresis band** so noise cannot flip them (e.g. the veto engages at
     ≥ X and releases below X − h).
   - **Moisture unavailable or stale** (the 3 h OR-of-{moisture, temp, EC} liveness gate)
     → the guard is inactive and weather decides alone. Fail-safe = the weather tree, never
     fail-`heavy`.
   - **Evaluation order:** weather tree → dry bump → forecast-rain downgrade → **wet veto**
     (last, hard). The downgrade can still act on a bumped zone, and nothing can override a
     veto (architecture §3.2 pseudocode).
3. **Calibration strategy:** each WH52 is recalibrated **on the gateway** against its own
   bed's soil: 0 %AD = air-dried sample, 100 %AD = the same sample saturated
   (`soil_calibration_log.md` §4).
   - The gateway fixes sensor error, including CH3's span error; per-zone HA thresholds
     absorb spot differences.
   - This supersedes the "moisture native 0–100 %, no calibration map" note above.
   - Redo a probe's calibration whenever it moves to a different substrate.
4. **Monitor role (new; design deferred to after drip install / go-live):** use moisture
   to evaluate effectiveness:
   - did the water land (a rise after the run)?
   - how long did it last (the dry-down rate)?
   - **"watered but no rise"** as a fault signal for a valve, emitter or line.
5. **Shadow evaluation offline, not live.** `decision_criteria` already records every
   input (moisture % + sensor count, rain, temp, forecast, thresholds, branch). So "what
   would a moisture ladder have chosen?" can be recomputed later from the DB with *any*
   threshold set, retroactively. The routine therefore **drops the live ladder**; the ADR-018
   recording is the evidence base for revisiting moisture's authority after a season.
6. **Knife-edge (root cause d):** with weather primary, the rain gates (`rain_min` etc.)
   are single-value thresholds again. Keep ADR-021's intent: **hysteresis on the rain
   gates** (band open). The wet veto backstops the worst case: a knife-edge "water" call
   on genuinely wet soil gets skipped.

**How the 2026-08-18 root causes are handled now:**

| Root cause | Handled by |
|------------|------------|
| (a) temperature lag | de-lagged, window-aware forecast high |
| (b) no current-conditions path | current-rain skip |
| (c) DWD missed the local downpour | moisture **wet veto**, plus the WH40 local gauge (ADR-021 Phase C) |
| (d) knife-edge | rain-gate hysteresis, with the veto as backstop |

The failure day itself (cool, overcast, ~10 mm local downpour) resolves to **`off` via the
wet veto** provided the downpour wetted the probe zone. That is the outcome ADR-021 required,
reached through the guard instead of the ladder.

**Helper changes:**
- **Retire** the 48 `input_number.zone_{1-4}_{season}_{off,light,normal}_moisture_min`. In
  code they are only referenced by `zone_helpers.yaml` and `decision_scripts.yaml` (verified
  2026-09-23). The unbuilt fert-eligibility *design* (architecture §2.2) used
  `normal_min ≤ moisture < off_min` and is re-pointed to `dry_bump < moisture < wet_veto`.
- **Replace** them with per-zone `input_number.zone_N_moisture_wet_veto`,
  `input_number.zone_N_moisture_dry_bump`, `input_boolean.zone_N_moisture_dry_bump_enabled`
  and a shared hysteresis helper. All are **RestoreEntity** (no `initial:`) and
  season-agnostic (soil wetness is not seasonal); add per-season values only if data demand it.
- **Values are set after the §4 gateway calibration**, on the new scale, from field behaviour.
- **Unset = disabled** (operator, 2026-09-23): a `wet_veto` / `dry_bump` of **0** means
  disabled. That's the first-boot value of a RestoreEntity helper with `min: 0`. The
  hysteresis lives in `binary_sensor.zone_N_moisture_wet` / `_dry` (built S2-rework).
- This **shrinks "Phase 2b"**: only the weather threshold helpers still need the RestoreEntity
  conversion.

**Consequences:**
- **Lower go-live risk.** Behaviour stays close to the deployed weather tree plus a veto,
  so the Phase-4 `window_check` rewire becomes low-risk.
- **Bounded sensor risk.** Sensor noise, placement or calibration drift can at most skip a
  cycle on wet soil; it cannot drive under- or over-watering.
- **Given up:** moisture-driven intensity. A dry-soil / cool-weather case gets only what
  the weather says unless dry-bump is enabled.
- **SEN0600 (ADR-021 Phase B) is DEFERRED indefinitely** (operator, 2026-09-23). The WH52s
  cover the guard and monitor roles, while the wired sensors add RS-485 + R10 pulse-poll
  complexity. The pulse-poll design above stays on record for if they're ever revisited,
  after a season of recorded data.
- **UI:** the "Current Status" tile = weather tree + guard state; the Soil-Moisture card
  shows the calibrated % with the veto line (ui_design §7 #1 respec).

**Alternatives considered:**
- *Keep moisture primary with rescaled thresholds + wide hysteresis.* Not chosen for now:
  the rungs would still be about the size of the noise, the lower anchor is unknown, and the
  spot spread dominates. Revisit with a season of recorded data (Decision 5).
- *In-ground plateau/peak gateway anchors.* Rejected: they mix sensor and spot error, clip
  at the dry end, and lock in provisional numbers.
- *Oven-dried / weighed lab calibration.* Rejected as too involved for two anchors.
- *Per-probe normalisation in HA only.* Not chosen: HA only sees whole-% values after the
  gateway has rounded them. Gateway calibration recovers resolution and fixes CH3 at the
  source; HA still carries the per-zone threshold layer.

**Open items:**
- `wet_veto` per zone (after the §4 calibration).
- Hysteresis bands (veto and rain gates).
- Whether and when to enable dry-bump.
- Design of the effectiveness monitor.

**Revisable:** all of it. Moisture's authority is explicitly up for review after one
season of recorded decisions.
...
* **2026-09-23**: **§3.6 S5 built: decision recording on `zone_runs` + weather tables in the export.**
  - **Operator choices:** the writer buffer (over season-only), and aligning the Event 1
    weather columns.
  - **Schema:** `zone_runs` + `season` (CHECK, nullable) + `decision_criteria` (JSON TEXT).
    - **Gotcha fixed at the root:** `CREATE TABLE IF NOT EXISTS` never alters an existing
      table, so the new columns would never have reached the Green's `watering_ops.db`.
    - `db_schema_init.py` now has an idempotent `COLUMN_MIGRATIONS` step (`PRAGMA
      table_info` → `ALTER TABLE ADD COLUMN`); its log line names what it added. Only
      nullable columns fit; a CHECK change or a NOT NULL column still needs a rebuild
      (`db_setup_guide.md` Step 3).
  - **Decision → run row.** `state_window_check` fires **Event 7
    `watering_decision_applied`** (window + the applied `zone_decisions`) immediately before
    the preflight transition, never on the all-off path.
    - `db_writer` holds it as the single pending decision. It attaches it at Event 1 only if
      ≤ 10 min old, so a parked Dev-Tools run can't leak into a later cycle. At Event 3 it
      writes that zone's criteria as compact, key-sorted JSON.
    - It is in memory like the dose buffer: an AppDaemon restart mid-cycle → NULL criteria.
    - Event 3 carries `season` from `input_select.zone_N_season`. An unknown value → NULL,
      and the row is never rejected over decision data.
  - **Event 1 alignment:** `watering_cycles.rainfall_24h_mm` / `_72h_mm` now come from
    `sensor.rain_24h` / `rain_72h` (the decision's WH40-or-DWD source), and `temp_high_c`
    from the window's forecast high. They were BrightSky rain + yesterday's high; older
    rows keep the old meaning (dated in architecture v1.11.0).
  - **Export:** `DbSeasonalExport` also writes `weather_snapshots_YYYY.csv` +
    `zone_decisions_YYYY.csv` from `watering_weather.db`.
    - The DB is opened read-only (`mode=ro`).
    - The year filter runs on the ISO `observed_at`; the legacy `+02:00` rows still fall
      in the right year.
    - A missing or unreadable weather DB skips only those two files, with a `warning`
      audit row.
    - `_export_table` now shares a `_write_csv` helper with the weather export.
  - **Tests (all passing):** `test_db_schema_init.py` NEW (11); `test_db_writer.py` 39
    (+15); `test_db_export.py` 22 (+11); `tests/ha_templates/test_s5_decision_recording.py`
    NEW (14: real routine output → Event 7 template → the REAL `DbWriter` → `zone_runs`).
    The template suite is 8 files / 168 checks.
  - **Docs:** architecture v1.11.0 §13 (schema, Event 1/3, new Event 7, §13.5), impl_roadmap
    §3.5 + §3.6 S5, Test 10.10 (live checks), `db_setup_guide.md`, START_HERE.
  - **Status:** not yet deployed. After the pull, restart the AppDaemon add-on (schema
    change, per the setup guide), then run Test 10.10. A real run row needs the ESP32 online.
* **2026-09-23**: **§3.6 S4 built: `state_window_check` applies `compute_zone_programs`.**
  - **Change:** the inline ADR-020 weather tree + cadence copy are deleted from
    `state_window_check` (`state_scripts.yaml`). It calls the routine for
    `input_select.active_watering_window` and applies the programs **by `zone_id`**. There is
    now one decision source for recording (S3) and actuation.
  - **Fail-safe:** a missing routine (checked first), a call error, an empty response, ≠ 4
    zones, a duplicate zone or an unknown program → every zone `off` + an `error` log →
    idle (A1-guarded). The due-day anchor is untouched, so due zones retry.
  - **A documented exception to the no-`continue_on_error` rule** (see the ADR-015
    "Principle enshrined" note).
  - **Logging:** the D-A warning is now keyed on the routine's `weather_available`. A new
    per-window `info` breadcrumb `Programs (<window>): Z1 … · Z2 off (<reason>) …` is the
    only record of a manual cycle's decision.
  - **Test:** `tests/ha_templates/test_s4_window_check.py` has 35 checks: real routine
    output → extraction, 7 defect shapes, the missing-routine guard and structure. The
    suite is 7 files / 154 checks, all passing.
  - **Watering behaviour at go-live** (all Amendment A, already recorded by S3):
    - temperature: window-aware forecast high (≥) instead of the 3-day average (>);
    - rain: the WH40-or-DWD source;
    - the current-rain skip and the forecast downgrade;
    - the moisture guard (inert at 0).
    Cadence, booster and multipliers are unchanged.
  - **Stale comments re-pointed** to the routine: `decision_scripts.yaml`,
    `zone_helpers.yaml`, `zone_scripts.yaml`, `derived_sensors.yaml`, `dwd_brightsky.yaml`.
  - **Status:** not yet deployed. It will be pushed together with `0d528db`
    (`observed_at` UTC). The live Dev-Tools pass (parked) is in Test 4.24.
* **2026-09-23**: **S3 validated END-TO-END live.**
  - **Deploy:** after the whitelist fix (`b7e75af`): pull, then an AppDaemon add-on restart.
    The log showed `watering_weather schema verified/created … (2 tables present)` and
    `WeatherLogger ready`.
  - **Result:** a manual recorder fire at 01:20:54 Berlin wrote `snapshot_id` 1 (a **test
    row**) + 4 `zone_decisions`, matching the prediction exactly. `weather_available` 1,
    forecast high 18.0, all zones `warm` → `normal` → `off` (zones disabled), moisture
    19.5 / 2 · 26.0 / 1 · NULL · NULL.
  - **Also learned:**
    - A fire during an AppDaemon restart is simply lost (no listener); the 01:18:43 fire
      was.
    - SQLite Web serves one configured file, so it must be pointed at `watering_weather.db`.
    - `temp_c` is NULL shortly after an HA restart until BrightSky current-weather fetches.
  - **Fixed: `observed_at` is now UTC.** It had been stored with the local offset
    (`+02:00`, from `now()`), contrary to `weather_schema.sql` / §13.3.1 and inconsistent
    with `watering_ops`. `compute_zone_programs` now uses `utcnow()`. `snapshot_id` 1 (the
    test row) predates the fix and keeps `+02:00`; it is still unambiguous.
    - New regression checks: `test_temp_horizon` renders the real template with a UTC
      clock; `test_s3_event_chain` asserts the UTC value is stored verbatim.
    - Suite: 6 files / 119 checks, all passing.
  - **Still open:** an unexplained "Unexpected error" in
    `bright_sky_warm_slow_rest_sensors_on_start` at startup. Its fetches still succeeded;
    the traceback is needed.
* **2026-09-23**: **S3 live fire exposed two S1 deploy gaps in the AppDaemon half (fixed).**
  HA side: the recorder and routine ran clean, zero HA errors.
  - **(1) AppDaemon import failure.** Both weather apps failed with
    `ModuleNotFoundError: No module named 'weather_logger'` / `'weather_db_schema_init'`.
    - AppDaemon builds its import paths at **add-on startup**. `watering_weather/` was
      created by a pull while it was running, so the folder was detected but never
      importable. An HA restart doesn't restart the add-on.
    - **Fix:** one AppDaemon add-on restart. After it, `WeatherLogger ready` appeared, and
      the logger received a **fully correct Event-6 payload** (all snapshot columns + `raw`),
      proving the `response_variable` round-trip and the contract live.
  - **(2) `no such table: weather_snapshots`.** `docs/weather_schema.sql` was not in the
    public-mirror whitelist (`.github/sanitize.py`). The pull script's copy step therefore
    WARN-skipped (outside the visible log tail), the schema init logged "Schema file not
    found", and the DB was never created.
    - **Fix:** whitelisted. Verified by running the sanitizer locally: the file is
      published byte-identical, and `tests/` stays private.
  - **Both are new START_HERE §3 gotchas.**
  - **Process change:** any deploy touching `appdaemon/` now includes getting the operator
    to confirm each app's AppDaemon "ready" / "schema verified" line.
  - The earlier "S1 deployed & verified" covered the HA half only; corrected in impl_roadmap.
  - **Next:** pull, restart the AppDaemon add-on, confirm the schema line, fire again, and
    inspect the rows.
* **2026-09-23**: **§3.6 S3 always-on recorder built, and S2 deployed + verified live.**
  - **S2 live:** all 80 weather thresholds kept their values through the RestoreEntity
    switch (script compare vs git: 0 mismatches). The guard is inert at 0 and the heartbeat
    fix is confirmed. The operator removed the orphaned registry entries.
  - **S3 — automation `watering_weather_snapshot_recorder`**
    (`packages/watering_weather/weather_recording.yaml`):
    - triggers at the morning/evening window starts with **no conditions** (ADR-018
      always-on: parked, winterized and disabled windows are all captured);
    - calls `compute_zone_programs(window = trigger id)` and fires Event 6;
    - adds the observational columns and `raw` (per-probe soil readings + liveness + tag,
      dew point, wind as reported).
    - `wind_speed_ms` stays NULL: the BrightSky wind sensor is labelled m/s but its request
      uses the default (`dwd`) units, probably km/h (START_HERE follow-up #10).
  - **S2 fix found while designing S3:** the routine judged the booster slot against
    `input_select.active_watering_window`, which only the scheduler sets, so it goes stale
    while parked. It now uses its `window` field (S4 passes the same value). It also records
    `window_enabled` / `manual_override` / `winterized` in the criteria; these don't affect
    the decision.
  - **New end-to-end offline test:** `tests/ha_templates/test_s3_event_chain.py`, 18 checks.
    It renders the real templates (zbase → zones → result → recorder `event_data`) and drives
    the **real** `WeatherLogger` into SQLite built from `weather_schema.sql`.
  - Suite: 6 files / 117 checks, all passing; the logger regression passes.
  - **Next:** deploy, fire once via "Run actions", and inspect the rows.
* **2026-09-23**: **§3.6 S2-rework built (ADR-021 Amendment A).** Code only; not yet
  deployed.
  - **`compute_zone_programs`** now runs weather tree → dry bump → forecast downgrade → wet
    veto (final, hard).
    - The moisture ladder and its thresholds are gone.
    - `decision_criteria` records the weather branch, `wp_weather`, the modifiers,
      `skip_reason`, the guard state + thresholds + hysteresis, and the weather thresholds,
      for offline replay.
    - Cadence, multiplier and the Event-6 shape are unchanged.
  - **New `packages/watering_state/moisture_guard.yaml`:**
    - per-zone `zone_N_moisture_wet_veto` / `_dry_bump` / `_dry_bump_enabled` + a shared
      `moisture_guard_hysteresis`, all RestoreEntity;
    - hysteresis-held template sensors `binary_sensor.zone_N_moisture_wet` / `_dry`. Each
      reads its own previous state via `this.state`, so after a restart the stricter engage
      threshold applies. Both are OFF when moisture is unknown/stale.
    - **Operator decision: 0 = disabled.** HA starts a helper with no stored value at its
      `min` (0), so the guard is inert until values are set after calibration.
  - **`zone_helpers.yaml`:** removed the 48 seasonal `*_moisture_min` helpers; dropped
    `initial:` from the 80 weather thresholds (RestoreEntity, the old "Phase 2b"); fixed the
    stale header.
    - Values carry over through HA's restore store. **Verify after the next restart**
      (fallback would be each helper's `min`).
  - **Tests promoted to `tests/ha_templates/`** (`run_all.py` + a shared `_harness.py`),
    all passing:
    - decision tree 31 (every weather branch, bump / downgrade / veto order, Aug-18
      regression, cadence, recorded criteria);
    - moisture guard 24 (engage / hold / release, restart, disabled, per-zone wiring);
    - Ecowitt templates 23; multiplier equivalence 10; temp horizon 7.
    - The AppDaemon `test_weather_logger` regression still passes.
  - **Deferred:** rain-gate hysteresis (Decision 6) until S3 data exists.
  - **Deploy:** pull + **full restart** (helpers are added and removed).
* **2026-09-23**: **First S1 deploy verified live; two `ecowitt.yaml` bugs fixed.**
  - **Deploy:** operator pulled; HA restarted 22:20:31 UTC.
  - **Verified live (read-only):**
    - park ON;
    - forecast sensors live (`forecast_temp_high_today` 18.0 / `_tomorrow` 15.8);
    - rain layer on DWD;
    - scripts loaded;
    - the `calculate_zone_runtime` → `resolve_program_multiplier` smoke test passed
      (operator, Dev Tools).
  - `moisture_wireless_4_zone` set to Blueberries (operator-approved).
  - **Bug 1 — HA template gotcha:** HA parses dict/JSON-looking template output back into a
    **native** type. `soil_moisture_pool`'s `| to_json` attribute therefore arrived as a dict,
    and every `zone_N_soil_moisture` reader's `from_json` errored, leaving all four
    `unavailable`.
    - **Fix:** the pool emits a native dict; the readers accept a mapping or a JSON string.
    - **Rule:** never round-trip template data through `to_json` / `from_json`.
  - **Bug 2:** after a restart, restored Ecowitt entities are `unavailable` but carry a fresh
    `last_reported`. The heartbeats therefore read a dead probe (CH1) and the uninstalled WH40
    as online.
    - **Fix:** all five heartbeats also require the state not to be `unknown` / `unavailable`.
  - **Offline regression test:** renders the actual templates, 17 checks, all passing.
  - **Neither bug was caught earlier:** the prior tests covered only `decision_scripts.yaml`,
    and HA's native-type parsing isn't reproduced by stock Jinja.
  - **Also found:** **WH52 CH1 (7876) offline since ~2026-09-22 20:45 UTC**, confirmed at the
    gateway; reset is START_HERE follow-up #9.
* **2026-09-23**: **ADR-021 Amendment A ACCEPTED: weather-primary trigger, moisture as guard
  + monitor.**
  - **Basis:** 3.5 weeks of live WH52 data (`soil_calibration_log.md` §6). The usable band
    is only ~10–13 pts (zone 1 ≈ 16–27 %, zone 2 ≈ 21–34 %), about the size of the noise
    (±1 dither, ±1–2 overnight temperature). Spot-to-spot spread within zone 1 exceeds the
    ladder rungs, the dry anchor is unknown, and the deployed 75/55/40 % thresholds were
    off-scale.
  - **Decision:** weather + cadence drive `wp`. The weather tree keeps the current-rain skip,
    the window-aware de-lagged temperature and the capped forecast downgrade. Moisture
    applies an optional dry bump and a final, hysteresis-held wet veto, and is inactive when
    stale.
  - **Shadow evaluation** of a moisture-led policy is done offline from the ADR-018 records;
    the live ladder is dropped.
  - **Helpers:** the 48 `*_moisture_min` ladder helpers are retired in favour of per-zone
    `wet_veto` / `dry_bump` (+ enable) + a shared hysteresis helper, all RestoreEntity. Fert
    eligibility is re-pointed to `dry_bump < moisture < wet_veto`.
  - **Calibration:** per-bed gateway calibration (air-dry / saturated sample,
    `soil_calibration_log.md` §4).
  - **SEN0600** (ADR-021 Phase B) is **DEFERRED indefinitely**.
  - **Build steps:** the weather-DB build steps are renamed to impl_roadmap §3.6
    **S1–S5**; the next is **S2-rework** of `compute_zone_programs`.
  - **Docs cascaded:** architecture v1.10.0, entity_reference, impl_roadmap §3.6/§6.2,
    test_scenarios Test 4.24, ui_design §7, START_HERE.
  - No code changed.
* **2026-08-31**: **ADR-018/ADR-021 Phase 2 code-review remediation + WH52 liveness-gate
  correction (code-only on local `main`, NOT deployed; system parked).** Six review findings on
  the 6-commit weather-DB/moisture-primary delta, plus a live-data correction. **#1 (payload
  contract):** `script.compute_zone_programs` returned the per-zone list as `zones` with snapshot
  floats nested under `result.snapshot`, but `weather_logger` reads `zone_decisions` and flat
  top-level snapshot columns — every Phase-3 snapshot would have been rejected. Flattened `result`
  to the logger contract (architecture §13.3.1 / `weather_schema.sql`); the 8 per-zone keys already
  matched. **#2 (temp horizon):** `brightsky_forecast_temp_high` uses a 04:00 rollover → returns
  TOMORROW at both decision windows. Made the de-lagged temp **window-aware** (an ADR-021
  refinement): widened the dedicated BrightSky fetch to now → tomorrow 23:00 and added
  `sensor.forecast_temp_high_{today,tomorrow}`; the routine reads today for the morning window,
  tomorrow for the evening (records `temp_high_horizon` in `decision_criteria`). The 04:00-rollover
  sensor is now dashboard-only. Rain/POP window-awareness tracked as START_HERE follow-up #6.
  **#4:** the `soil_moisture_pool` loops gated on an `unknown/unavailable` denylist, so any other
  non-numeric would error `| float` (no default) and null the whole attribute → all zones drop to
  weather-only; switched to an `is_number()` gate. **#5:** `forecast_rain_today` `state_class`
  `total` → `measurement` (non-monotonic rest-of-today sum). **#3:** verified the EC entity ids live
  and added the raw `gw3000a_soil_*` channel entities to `entity_reference.md` (canonical), plus
  fixed a `forecast_pop_today`/`_rain_today` id drift there. **#6** (four copy-paste multiplier
  calls) left as-is by choice. **Liveness gate:** the HA Ecowitt integration only advances
  `last_reported` on a value change, so the moisture-keyed `WH52 Soil N Online` heartbeats
  false-stale a healthy-but-flat probe; rewrote each to OR the freshness of {moisture, temp, EC}
  over a 3 h window (a midnight "feed died" flag turned out to be flat integers, not dropped
  reports). WH40 heartbeat has the same flaw — tagged follow-up. Validated: yamllint + PyYAML parse
  + offline jinja tests (30-branch tree, 56/9 multiplier-equivalence, 7 temp-horizon) all pass.
