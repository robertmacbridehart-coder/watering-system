# Watering System Architecture v1.2.2
...
## 6. Automation Structure

### 6.1 Master State Machine
**File:** `home-assistant/packages/watering_state_machine.yaml`
...
```yaml
automation:
  - alias: "Watering System - State Machine"
    trigger:
      - platform: state
        entity_id: input_select.watering_system_state
    action:
      - choose:
          - conditions: [state = 'window_check']
            sequence: [call script.window_check]
          
          - conditions: [state = 'preflight_check']
            sequence: [call script.preflight_check]
          
          - conditions: [state = 'watering_plain']
            sequence: [call script.watering_plain_sequence]
          
          # [etc. for each state]
```

### 6.2 State Transition Scripts
**File:** `home-assistant/packages/watering_scripts.yaml`
...
**File:** `home-assistant/packages/zone_control.yaml`
...
**File:** `home-assistant/packages/fert_control.yaml`
...
**File:** `home-assistant/packages/watering_safety.yaml`
...
### 6.6 Sensor Reading Scheduler
**File:** `home-assistant/packages/sensor_scheduler.yaml`

Periodic soil sensor reading (future):
