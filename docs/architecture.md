# Watering System Architecture v1.11.0
...
- **Sensors:** Float switches (GPIO33=Low, GPIO32=Low-Low); soil sensors = **4× wireless Ecowitt WH52** (moisture/temp/EC), installed 2026-08-29. The wired RS-485 SEN0600 option is **DEFERRED indefinitely** (ADR-021 Amendment A); see §9.1.
- **Weather:** an Ecowitt **GW3000** gateway (868 MHz) is installed (2026-08-29) and carries the WH52s. The WH40 tipping-bucket rain gauge is pending; §9.4 records the originally planned GW1200 + WS90 + WH40 set.
...
- **Primary:** Soil moisture check (if a fresh sensor is available)
  - Range: `zone_N_moisture_dry_bump < moisture < zone_N_moisture_wet_veto`. This means not
    so dry that the zone needs a catch-up watering, and not so wet that the watering is
    vetoed. *Re-pointed 2026-09-23 (ADR-021 Amendment A): the per-season
    `normal/off_moisture_min` ladder helpers were retired. Fert scripts are not yet built
    (Phase 3.3), so this is a design change only.*
...
`booster` (Phase 7) is a SYSTEM-set value only — the decision routine (§3.2) sets it to
...
on day N"). Both are consumed by `script.compute_zone_programs` (§3.2). See §4.2.
...
produce the final program. **One decision source (§3.6 S4, 2026-09-23):** the whole
selection lives in the side-effect-free `script.compute_zone_programs`;
`state_window_check` calls it for the active window and **applies** the returned programs
(the always-on S3 recorder calls the same routine to record every window). If the routine
is not loaded, errors, or returns a malformed result, `state_window_check` **fails safe**: every zone
`off`, an `error` log, back to idle — one window skipped, the cadence anchor untouched, so
due zones retry next window. The call carries `continue_on_error` as a **documented
exception to the ADR-015 no-`continue_on_error` rule** (the routine actuates nothing; aborting would leave the machine
stuck in `window_check`, and the scheduler only starts from idle). Intensity is *what* to water; cadence is *when*. (Closes
ADR-015 D-C, the deferred cadence gate, now that `sensor.zone_N_watering` supplies the
last-main-dose anchor.)

> **STATUS — intensity per ADR-021 as amended by Amendment A (ACCEPTED 2026-09-23).**
>
> - **Design:** **weather + cadence is the PRIMARY trigger**; soil moisture is a **guard**
>   (a wet veto, plus an optional dry bump) and a **monitor**, not a ladder. The
>   moisture-primary ladder of ADR-021 v1 was **reversed** after 3.5 weeks of WH52 data
>   showed a ~10-pt usable band about the size of the sensor noise (see ADR-021 Amendment A;
>   `soil_calibration_log.md` §6).
> - **Code today:** `script.compute_zone_programs` implements this design (S2-rework,
>   deployed 2026-09-23); `state_window_check` **applies** it since the S4 rewire
>   (2026-09-23; live once pulled). Parked until go-live, so nothing is actuated yet.
> - **Unchanged:** the **cadence/booster layer (ADR-020)**.
> - **Recording:** decision recording is ADR-018 (`zone_decisions` / `decision_criteria`),
>   which records every input so a moisture-led policy can be re-evaluated offline.

```python
# Pseudocode for zone program determination (script.compute_zone_programs,
# applied by state_window_check)
def select_zone_program(zone_id, active_window):
    # ---- intensity -> wp (ADR-021 Amendment A: WEATHER-PRIMARY + moisture GUARD) ----
    # Ladder order (dry->wet needs less water): heavy > normal > light > off.
    t = get_zone_thresholds(zone_id, season)        # weather: rain_off/light/min mm, temp_heavy/normal c
    g = get_moisture_guard(zone_id)                 # wet_veto, dry_bump, dry_bump_enabled, hysteresis
    temp_high = (forecast_temp_high_today if active_window == "morning"
                 else forecast_temp_high_tomorrow)  # DE-LAGGED + WINDOW-AWARE (NOT temp_avg_high_3day)
    moisture  = sensor.zone_{id}_soil_moisture      # gateway-calibrated %, avg of FRESH sensors
                                                    # mapped to the zone; unknown if none fresh
                                                    # (3 h OR-of-{moisture,temp,EC} liveness gate)

    # 1. WEATHER TREE (primary)
    if not weather_available:                                   wp = "normal"  # D-A
    elif rain_now:                                              wp = "off"     # current-rain hard skip
    elif rain_72h > t.rain_off_mm:                              wp = "off"
    elif rain_24h > t.rain_light_mm:                            wp = "light"
    elif temp_high is unknown:                                  wp = "normal"  # neutral, never heavy
    elif temp_high >= t.temp_heavy_c and rain_72h < t.rain_min_mm: wp = "heavy"
    elif temp_high >= t.temp_normal_c:                          wp = "normal"
    else:                                                       wp = "light"

    # 2. DRY BUMP (optional; disabled by default): clearly dry soil -> +1 step, capped at heavy
    if moisture is known and g.dry_bump_enabled and wp != "off" and moisture <= g.dry_bump:
        wp = step_up(wp)

    # 3. FORECAST-RAIN DOWNGRADE: <=2 steps, FLOORED so a hot/dry zone still waters
    #    (heavy->light, normal->off, light->off). Gated on rest-of-today POP + volume.
    if wp != "off" and forecast_pop_today > 80 and forecast_rain_today >= 5.0:
        wp = downgrade(wp, max_steps=2, heavy_floor="light")

    # 4. WET VETO (final, hard): clearly wet soil -> off. Hysteresis-held so noise cannot
    #    flip it (engage at >= wet_veto, release below wet_veto - hysteresis).
    #    Inactive when moisture is unknown/stale -> the weather tree decides alone.
    if moisture is known and wet_veto_active(zone_id, moisture, g):
        wp = "off"                                  # skip_reason: moisture_wet
...
**Notes (ADR-021 Amendment A intensity).**
- **Weather is primary; moisture guards.** The four 2026-08-18 root causes are each covered:
  - (a) temperature lag → the de-lagged, **window-aware** forecast high;
  - (b) no current-conditions path → the current-rain hard skip;
  - (c) DWD missed the local downpour → the **wet veto** (plus the WH40 local gauge once
    installed);
  - (d) knife-edge thresholds → hysteresis on the rain gates, with the veto as backstop.
- **The failure day** (cool, overcast, ~10 mm local downpour) resolves to `off` **via the
  wet veto**, provided the downpour wetted the probe zone.
- **Why a guard, not a ladder.** Rain and watering move the probes +10–40 pts, well clear of
  the noise. The absolute level within the ~10-pt band does not, so moisture only gets
  coarse, hysteresis-held authority.
- **Step order matters.** Dry bump before the forecast downgrade; the veto last and hard.
- **Unknown or stale moisture** disables the guard. The weather tree then decides alone:
  fail-safe, never fail-`heavy`.
- **Every input is recorded** via **ADR-018** (`zone_decisions` / `decision_criteria`),
  including moisture % + contributing-sensor count. A moisture-led policy can therefore be
  re-evaluated offline against any threshold set before moisture is given more authority.
- **Status:** implemented in `compute_zone_programs` (S2-rework) and applied by
  `state_window_check` (S4); *the cadence notes below are LIVE (ADR-020).*
...
as a single dose, and the decision routine schedules no booster.
...
**ADR-021 + Amendment A additions:**
- **RETIRE the moisture ladder helpers** (Amendment A, 2026-09-23): the 48
  `input_number.zone_N_{season}_{off,light,normal}_moisture_min`. Only `zone_helpers.yaml`
  and `decision_scripts.yaml` reference them in code (verified 2026-09-23). The fert
  eligibility *design* (§2.2) is re-pointed to the guard helpers below.
- **Per-zone moisture-guard helpers**, **BUILT** 2026-09-23 in
  `packages/watering_state/moisture_guard.yaml` (season-agnostic; **RestoreEntity**, no
  `initial:`):
  - `input_number.zone_N_moisture_wet_veto`
  - `input_number.zone_N_moisture_dry_bump`
  - `input_boolean.zone_N_moisture_dry_bump_enabled` (first boot off)
  - `input_number.moisture_guard_hysteresis` (shared)

  **A threshold of 0 = disabled.** That is the first-boot value, so the guard is inert
  until set. Values are set on the **gateway-calibrated** scale (`soil_calibration_log.md`
  §4) from field behaviour.
- **Guard sensors** `binary_sensor.zone_N_moisture_wet` / `_dry` implement the hysteresis
  (`wet_veto_active()` in the §3.2 pseudocode). Each reads its own previous state via
  `this.state`, and both are OFF when moisture is unknown/stale or the threshold is 0.
  `compute_zone_programs` reads these rather than re-deriving thresholds.
- **Weather threshold helpers** (`zone_N_{season}_{rain_*,temp_*}`) → **pure RestoreEntity**
  (drop `initial:`) so operator tuning survives a restart (ADR-017 pattern; closes
  follow-up #4(e)).
- **Wireless-sensor zone tags** `input_select.moisture_wireless_N_zone` (one per movable
  WH52; Raspberries/Blueberries/Vegetables/Unassigned): **BUILT** in
  `packages/weather/ecowitt.yaml` (352caa9) and feed the per-zone
  `sensor.zone_N_soil_moisture` averaging (§9.1).
...
> **RETIRED 2026-09-23 (ADR-021 Amendment A).**
>
> - **What was retired:** the 3 × 4-season × 4-zone
>   `zone_X_SEASON_{normal,light,off}_moisture_min` ladder, with generic "berry-optimized"
>   initials of 35–80 %.
> - **Why:** this soil's real in-ground range is ~12–34 % on the factory scale
>   (`soil_calibration_log.md` §6), so those values were off-scale.
> - **Fert eligibility now uses** the per-zone guard helpers (§4.2):
>   `zone_N_moisture_dry_bump < moisture < zone_N_moisture_wet_veto`.
> - **Values** are set on the gateway-calibrated scale after the §4 bed-sample calibration.
...
Script: state_window_check
  - Call script.compute_zone_programs(window) (§3.2): weather tree + moisture
    guard (intensity: off/light/normal/heavy), then the cadence gate: per-zone
    enable + interval (days since last main dose from sensor.zone_N_watering)
    + heavy mid-interval booster
  - Set zone programs (off/light/normal/heavy/booster); routine failure ->
    all off -> idle (fail-safe)
  ↓
...
> **Status (ADR-021 as amended by Amendment A, 2026-09-23).**
>
> - **Hardware:** **4× wireless Ecowitt WH52**, installed 2026-08-29 on the GW3000. The
>   3× wired SEN0600 (Option A) are **DEFERRED indefinitely** (operator, 2026-09-23): the WH52s
>   cover the guard and monitor roles, and RS-485 + the R10 pulse-poll add complexity for
>   little gain. Revisit after a season of recorded decisions.
> - **Role:** soil moisture is a **GUARD** (a wet veto, plus an optional dry bump) and a
>   **MONITOR** for the §3.2 weather-primary tree, not the primary input.
> - **Mapping:** the movable WH52s carry a runtime zone tag
>   (`input_select.moisture_wireless_N_zone`). `sensor.zone_N_soil_moisture` = the
>   **average** of the fresh sensors mapped to the zone. Zones 3 & 4 share the veg bed, so
>   both read the zone-3 pool.
> - **Liveness:** the HA Ecowitt integration never marks a dead probe `unavailable`, so a
>   3 h OR-of-{moisture, temp, EC} liveness gate is used (`ecowitt.yaml`).
> - **Calibration:** the earlier claim that WH52 moisture is "native 0–100 %, no calibration"
>   is **superseded**. Each probe is recalibrated **on the gateway** against its own bed's
>   soil (0 %AD = air-dried sample, 100 %AD = saturated sample), and redone whenever a probe
>   moves to another substrate. See `soil_calibration_log.md` §4.

**Integration goals:**
- **Guard:** wet veto (moisture ≥ `zone_N_moisture_wet_veto` → `off`, hysteresis-held); an
  optional dry bump (≤ `zone_N_moisture_dry_bump` → +1 step)
- **Monitor (after the drip install):** post-run rise and dry-down rate, plus a "watered but
  no rise" fault signal for a valve, emitter or line
- **Fail-safe:** with no fresh sensor, the guard is inactive and the weather tree decides alone
- **Record** every input (ADR-018) so moisture's authority can be re-evaluated offline

**Option A — Wired RS-485 (DEFERRED indefinitely, ADR-021 Amendment A):**
...
- ~~**RESOLVED (ADR-021):** adopted as a hybrid~~. **Superseded 2026-09-23 (Amendment A):**
  WH52-only; the SEN0600 wired half is deferred indefinitely. **Depth caveat still stands:**
  the WH52 senses the top ~10–15 cm, which under-represents the deeper-rooted raspberry/kiwi
  root zone. That is acceptable for a wet-veto guard, and is one reason moisture does not
  drive intensity.
...
| `rainfall_24h_mm` | DECIMAL(5,1) | Weather snapshot at preflight: the decision's `sensor.rain_24h` (WH40-or-DWD) since 2026-09-23; `brightsky_rain_24h` before |
| `rainfall_72h_mm` | DECIMAL(5,1) | As above, `sensor.rain_72h` |
| `temp_high_c` | DECIMAL(4,1) | The window's forecast high (morning → today, evening → tomorrow) since 2026-09-23; yesterday's high (`brightsky_temp_high_yesterday`) before |
...
| `season` | VARCHAR(10)\|null | Zone season at run time (`spring`/`summer`/`fall`/`winter`); §3.6 S5 |
| `decision_criteria` | TEXT (JSON)\|null | The applied decision for this zone: `compute_zone_programs`' criteria (branch, skip reason, modifiers, weather inputs + thresholds, moisture guard, cadence). NULL for pre-S5 rows or when the writer did not hold the decision (AppDaemon restart mid-cycle); §3.6 S5 |

Both S5 columns were added to the live table by the bootstrap's idempotent column
migration (`db_schema_init.py` `COLUMN_MIGRATIONS`, `ALTER TABLE ADD COLUMN`); a
fresh database gets them from `db_schema.sql`.
...
| (buffer; lands on `zone_runs`) | Window check hands a cycle to preflight | `watering_decision_applied` (Event 7) |
...
| `rainfall_24h_mm` | float\|null | no | `rainfall_24h_mm` (`sensor.rain_24h`, the decision's source) |
| `rainfall_72h_mm` | float\|null | no | `rainfall_72h_mm` (`sensor.rain_72h`) |
| `temp_high_c` | float\|null | no | `temp_high_c` (`sensor.forecast_temp_high_today`/`_tomorrow` by window) |

AppDaemon: INSERT the row; record `cycle_uuid -> cycle_id` (the new `lastrowid`);
set `binary_sensor.watering_cycle_active` on; attach the buffered Event 7 decision
to this cycle if it arrived within the last 10 min (else discard it as stale).
`end_time`, `outcome`, and `notes` stay NULL until Event 4.
...
| `season` | str\|null | no | `season` (`input_select.zone_N_season` at run time; an unknown value → NULL, never a reject) |

`decision_criteria` is **not** in this payload: AppDaemon takes it from the cycle's
buffered Event 7 decision, by `zone_id` (NULL when there is none).
...
**Event 6 — `watering_weather_snapshot`** → INSERT `weather_snapshots` (1) + `zone_decisions` (N)

Writes to the SEPARATE weather-observations DB (`watering_weather.db`,
`docs/weather_schema.sql`), NOT `watering_ops`. Fired twice-daily by the always-on
snapshot automation on the `morning_window_start` / `evening_window_start` helpers —
regardless of system state (parked in `manual_override`, winterized, mid-cycle,
idle) — so skip/parked days are captured (ADR-018). The per-zone decisions come from
the shared `script.compute_zone_programs` routine (ADR-021 as amended by Amendment A:
weather-primary + moisture guard).
- **Producer (built 2026-09-23, impl_roadmap §3.6 S3):** automation
  `watering_weather_snapshot_recorder` in `packages/watering_weather/weather_recording.yaml`.
  - It has **no conditions**, so it fires at both window starts in every state.
  - It calls the routine with `window` = the trigger id, maps the response into the
    event, and adds the observational columns (current temp, yesterday's high, 3-day
    average, humidity, pressure, cloud) plus `raw`.
- **Context flags:** each decision's criteria carries `window_enabled` /
  `manual_override` / `winterized`, which say whether the decision could have been
  applied.

This
event carries the FULL snapshot plus the four decisions, so it needs no cross-event
correlation. Consumer: `weather_logger.py`.

Top-level snapshot fields:

| Field | Type | Required | Maps to |
|-------|------|----------|---------|
| `observed_at` | str (UTC) | yes | `observed_at` |
| `window_name` | str | yes | `window_name` (`morning`/`evening`) |
| `source` | str | no (default `brightsky`) | `source` (`brightsky`/`local_station`/`mixed`) |
| `weather_available` | 0\|1 | no (default 1) | `weather_available` |
| `temp_c` | float\|null | no | `temp_c` |
| `temp_high_forecast_c` | float\|null | no | `temp_high_forecast_c` (the de-lagged high the tree uses) |
| `temp_high_yesterday_c` | float\|null | no | `temp_high_yesterday_c` |
| `temp_avg_high_3day_c` | float\|null | no | `temp_avg_high_3day_c` (the lagging metric, recorded for comparison) |
| `rain_24h_mm` | float\|null | no | `rain_24h_mm` (the SELECTED value, via the WH40/DWD abstraction) |
| `rain_72h_mm` | float\|null | no | `rain_72h_mm` |
| `raining_now` | 0\|1\|null | no | `raining_now` |
| `rain_source` | str\|null | no | `rain_source` (`wh40`/`dwd`/`none`) |
| `forecast_pop_today` | float\|null | no | `forecast_pop_today` |
| `forecast_rain_today_mm` | float\|null | no | `forecast_rain_today_mm` |
| `humidity_pct` | float\|null | no | `humidity_pct` |
| `pressure_hpa` | float\|null | no | `pressure_hpa` |
| `cloud_cover_pct` | float\|null | no | `cloud_cover_pct` |
| `wind_speed_ms` | float\|null | no | `wind_speed_ms` (**left NULL by the S3 recorder until the BrightSky wind unit is verified**; the value goes to `raw` as reported) |
| `raw` | dict\|str\|null | no | `raw` (JSON string). The S3 recorder writes: `soil.{1-4}` = moisture / temp_c / ec_us_cm / battery_v / online / zone_tag; `dew_point_c`; `wind_speed_as_reported` (+ a unit note); `recorder` version |
| `zone_decisions` | list (non-empty) | yes | → `zone_decisions` rows |

Each `zone_decisions[]` element:

| Field | Type | Required | Maps to |
|-------|------|----------|---------|
| `zone_id` | int 1–4 | yes | `zone_id` |
| `season` | str | yes | `season` (`spring`/`summer`/`fall`/`winter`) |
| `computed_program` | str | yes | `computed_program` (`off`/`light`/`normal`/`heavy`/`booster` — the FINAL program after cadence gates) |
| `program_multiplier` | float\|null | no | `program_multiplier` |
| `would_water` | 0\|1 | no (DERIVED `program != 'off'` if absent) | `would_water` |
| `moisture_pct` | float\|null | no | `moisture_pct` (zone aggregate; NULL → weather-only fallback) |
| `moisture_sensor_count` | int ≥0 | no (default 0) | `moisture_sensor_count` (0 = fallback used) |
| `decision_criteria` | dict\|str | yes | `decision_criteria` (JSON; see the key set in `weather_schema.sql`) |

AppDaemon: coerce types (empty/`unknown`/`unavailable` → NULL; dicts → JSON string);
validate the window/source/season/program vocabularies and a non-empty
`zone_decisions`; then INSERT the snapshot and all decisions in ONE transaction (FK
enforced, so a bad child rolls the parent back — no orphan snapshot). A rejected
payload is logged and skipped; the weather DB has no `system_events` table, so
rejections are NOT persisted as breadcrumb rows (fire-and-forget reporting, §13.1).

**Event 7 — `watering_decision_applied`** → buffered in memory; written onto `zone_runs` at Event 3

Fired by `state_window_check` just before it hands a cycle to preflight (never on
the all-off → idle path), §3.6 S5. It carries the decision `window_check` actually
applied; `cycle_uuid` does not exist yet (preflight mints it).

| Field | Type | Required | Maps to |
|-------|------|----------|---------|
| `window` | str | no | (logged only) |
| `zone_decisions` | list | yes | per item: `zone_id` + `decision_criteria` → that zone's `zone_runs.decision_criteria` (the `compute_zone_programs` list, same shape as Event 6's) |

AppDaemon (`db_writer`): hold it as the single pending decision (a newer Event 7
replaces an older one; malformed items are skipped); attach it to the cycle at the
next Event 1 if under 10 min old; at Event 3 write the zone's criteria as compact
JSON; drop it at Event 4. In memory only, like the dose buffer: an AppDaemon
restart mid-cycle leaves that cycle's `decision_criteria` NULL while the run rows
are still written. Scheduled windows are ALSO recorded in the weather DB (Event 6),
so a NULL here loses only a manual cycle's record.
...
- **Seasonal export:** the AppDaemon `db_export` app (`DbSeasonalExport`) exports
  all four tables as dated CSV files, plus the weather DB's `weather_snapshots` and
  `zone_decisions` (§3.6 S5; see "Weather DB" below). It is triggered by the HA event
...
- **Year-filtered:** each file holds only the rows whose UTC timestamp falls in
  ...
  is always complete.
- **Weather DB** (`/homeassistant/watering_weather.db`, `weather_db_path` in
  `apps.yaml`): `weather_snapshots_YYYY.csv` (filtered on the ISO `observed_at`) and
  `zone_decisions_YYYY.csv` (the decisions of those snapshots). Opened **read-only**
  (SQLite `mode=ro`): export only, nothing is deleted (long-term retention). It is
  secondary: if it is missing or unreadable, only those two files are skipped and
  the audit row is `warning`; the operational export still completes.
- **Audit:** each export writes one `system_events` row recording the outcome
  (`event_type = 'seasonal_export'`, `info` with per-table row counts incl. the
  weather tables, `warning` if the weather tables were skipped, or `critical` on
  failure). The export is otherwise read-only and never raises, so a
...
### Version 1.11.0
**2026-09-23**
**§3.6 S5: decision recording on `zone_runs` + weather tables in the seasonal export**
- **§13.2:** `zone_runs` gains `season` + `decision_criteria` (JSON), both nullable,
  added to the live DB by a new idempotent column migration in `db_schema_init.py`.
  The `watering_cycles` weather columns now record the decision's inputs (the
  selected rain; the window's forecast high) instead of BrightSky rain + yesterday's
  high (dated; older rows keep the old sources).
- **§13.3 / §13.3.1:** new **Event 7 `watering_decision_applied`** (window check →
  `db_writer` buffer → Event 1 attach → Event 3 write); Event 3 gains `season`;
  Event 1 source columns updated.
- **§13.5:** the seasonal export also writes `weather_snapshots_YYYY.csv` +
  `zone_decisions_YYYY.csv` from the weather DB (read-only; a missing weather DB
  only skips those and marks the audit `warning`).

### Version 1.10.1
**2026-09-23**
**§3.6 S4: `state_window_check` applies the shared decision routine**
- **§3.2:** `state_window_check` no longer carries its own weather tree. It calls
  `script.compute_zone_programs` for the active window and applies the returned programs,
  so recording (S3) and actuation share one decision source.
- **Fail-safe:** a missing routine, a routine error or a malformed result → every zone
  `off`, an `error` log,
  back to idle. `continue_on_error` on the call is a documented exception to the ADR-015
  no-`continue_on_error` rule.
- A one-line per-window breadcrumb (`Programs (<window>): Z1 … · Z2 off (<reason>) …`) goes
  to `system_events`, which is the only record of a manual cycle's decision.
- STATUS banner, pseudocode header, §3.1 / §3.3 references and the §7.1 flow updated.

### Version 1.10.0
**2026-09-23**
**ADR-021 Amendment A accepted: weather-primary trigger, moisture as guard + monitor**
- **§3.2:** reversed the moisture-primary ladder.
  - `wp` comes from the weather tree, which keeps the current-rain skip, the de-lagged
    temperature (now **window-aware**: morning → today's forecast high, evening →
    tomorrow's) and the capped forecast-rain downgrade.
  - Moisture then applies an optional **dry bump** (+1) and a final, hysteresis-held
    **wet veto** (→ `off`), and is inactive when stale.
  - Updated the STATUS banner, pseudocode and notes. The ADR-020 cadence layer is unchanged.
- **§4.2 / §4.2.6:** retired the 48 `zone_N_{season}_{off,light,normal}_moisture_min`
  helpers. Planned replacements are per-zone `zone_N_moisture_wet_veto` / `_dry_bump` /
  `_dry_bump_enabled` + a shared hysteresis helper (RestoreEntity). The weather threshold
  helpers still go RestoreEntity. The zone tags are now BUILT.
- **§2.2:** fert eligibility moisture band re-pointed to `dry_bump < moisture < wet_veto`
  (fert scripts not yet built).
- **§1.2 / §9.1:**
  - Soil sensing = 4× WH52 on the installed GW3000.
  - SEN0600 wired option **DEFERRED indefinitely**.
  - The "native 0–100 %, no calibration" claim is superseded by per-bed gateway calibration
    (`soil_calibration_log.md` §4).
  - A 3 h OR-of-3 liveness gate is noted.
- Basis: 3.5 weeks of WH52 data (`soil_calibration_log.md` §6); full rationale in ADR-021
  Amendment A (programming-notes.md).
