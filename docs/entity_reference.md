...
| `sensor.mppt_load_power` | template (W) | Solar-card instantaneous load power = `mppt_load_current × mppt_battery_voltage` (the Victron MPPT exposes no native load-W/-Wh field). `state_class: measurement` → also feeds a planned Integration→daily Utility Meter for the Solar-card daily-usage series. `unavailable` when either source is (ESP32 offline). Built 2026-08-26. |
| `binary_sensor.esp32_comms` | template (connectivity) | Control-strip comms light. `on`/`off` mirror of the main-pump relay **R1** availability (`switch.watering_system_relay_1_main_pump`) — the same proxy the comms watchdog / `stop_main_pump` fail-fast use. **Display mirror only** (drives no control; the `error_comms_lost` watchdog stays authoritative); never reports `unavailable`. Built 2026-08-26. |
...
### ADR-021 (+ Amendment A) Entities — BUILT in repo / PLANNED

**Status (2026-09-23):** ADR-021 is **amended by Amendment A**: weather-primary, with moisture
as a guard + monitor.
- **BUILT** rows are committed and pushed (`ecowitt.yaml`, `dwd_brightsky.yaml`,
  `decision_scripts.yaml`). They reach the Green on the **next repo pull**; until you have
  pulled and verified, treat them as not live.
- **PLANNED** rows are the intended names for the S2-rework, and may change at
  implementation.
- The SEN0600 wired sensors are **deferred indefinitely**, so WH52 only.

| Entity ID | Entity Type | State | Purpose |
|-----------|-------------|-------|---------|
| `sensor.zone_{1-4}_soil_moisture` | template (%) | BUILT | Per-zone aggregate = **average of the fresh WH52s** mapped to the zone (attr `contributing_sensors`). A **guard** input (wet veto / dry bump), not primary. Zones 3 & 4 both read the "Vegetables" pool. `unknown` when no fresh sensor → guard inactive. |
| `sensor.soil_moisture_pool` | template | BUILT | Resolves tags + freshness once. State = count of fresh, tagged WH52; attr `zone_values_json`. |
| `binary_sensor.wh52_soil_{1-4}_online` | template (connectivity) | BUILT | Per-probe liveness: ON if any of moisture / temp / EC reported within **3 h** (the HA Ecowitt integration never marks a dead probe `unavailable`). |
| `input_select.moisture_wireless_{1-4}_zone` | input_select | BUILT | Runtime zone tag per movable WH52 (Raspberries / Blueberries / Vegetables / Unassigned). RestoreEntity, no `initial:` → **after the first pull, set `moisture_wireless_4_zone` = Blueberries once.** |
| `input_number.zone_{1-4}_moisture_wet_veto` | input_number (%) | BUILT | Wet veto: moisture ≥ value → `off` (hysteresis-held). **0 = disabled**, which is the first-boot value (RestoreEntity, no `initial:`), so the guard is inert until set. Set on the gateway-calibrated scale. `packages/watering_state/moisture_guard.yaml`. |
| `input_number.zone_{1-4}_moisture_dry_bump` | input_number (%) | BUILT | Optional dry bump: moisture ≤ value → +1 step (capped at heavy). **0 = disabled.** Also the lower bound of fert eligibility. RestoreEntity. |
| `input_boolean.zone_{1-4}_moisture_dry_bump_enabled` | input_boolean | BUILT | Enables the dry bump per zone (first boot off). RestoreEntity. |
| `input_number.moisture_guard_hysteresis` | input_number (pts) | BUILT | Shared hysteresis band for the veto and bump, 0–10 in 0.5 steps (first boot 0 = plain threshold). RestoreEntity. |
| `binary_sensor.zone_{1-4}_moisture_wet` | template (moisture) | BUILT | **ON = zone clearly wet → veto.** Engages at moisture ≥ `wet_veto`, releases below `wet_veto − hysteresis`, using its own previous state (`this.state`). OFF when moisture is unknown/stale or the veto is 0. Read by `compute_zone_programs`; also the dashboard's 🟢 guard dot. |
| `binary_sensor.zone_{1-4}_moisture_dry` | template | BUILT | **ON = zone clearly dry.** Engages at moisture ≤ `dry_bump`, releases above `dry_bump + hysteresis`. It only bumps if the zone's dry-bump enable is on. OFF when moisture is unknown/stale or the threshold is 0. Dashboard 🔴 dot. |
| `sensor.forecast_pop_today` | sensor (%) | BUILT | Rest-of-today (now → midnight) max hourly precipitation probability, from a DEDICATED BrightSky fetch (not the 04:00-rollover card fetch). Forecast-rain downgrade gate (> 80 %). *(Actual entity id — earlier drafts of this row said `brightsky_forecast_pop_today`.)* |
| `sensor.forecast_rain_today` | sensor (mm) | BUILT | Rest-of-today (now → midnight) summed forecast precipitation, same dedicated fetch. Downgrade gate (≥ 5 mm). `state_class: measurement`. Distinct from `brightsky_forecast_rain` (04:00-rollover card sensor). |
| `sensor.forecast_temp_high_today` | sensor (°C) | BUILT | Max hourly forecast temp over the REST of today (hour ≥ now), from the dedicated fetch (widened to now → tomorrow 23:00). De-lagged temp modifier for the **morning** window. `unknown` → neutral. |
| `sensor.forecast_temp_high_tomorrow` | sensor (°C) | BUILT | Max hourly forecast temp over the whole of tomorrow, same fetch. De-lagged temp modifier for the **evening** window (an evening watering runs into the next day). `unknown` → neutral. Supersedes the 04:00-rollover `brightsky_forecast_temp_high` in the decision path (that sensor is now dashboard-card only). |
| `sensor.zone_{1-4}_current_status` | template | PLANNED | The "Current Status" preview (rescoped item above): the live §3.2 program the zone would get if the window were now (weather tree + moisture-guard state). Dashboard only. |
...
#### Ecowitt WH52 raw channel entities — LIVE (hardware, on the GW3000A gateway)

These exist **now** (movable WH52 3-in-1 probes; verified live 2026-08-31) and are the raw
inputs the `sensor.zone_{1-4}_soil_moisture` aggregate + per-channel liveness gate read. `N` = 1–4
(channel). Channel→zone tagging is runtime via `input_select.moisture_wireless_{N}_zone`. The HA
Ecowitt integration NEVER marks a dead probe `unavailable` and only advances `last_reported` on a
value change — so liveness is gated on the OR of the three fast-changing fields (see
`ecowitt.yaml`), NOT the moisture timestamp.

| Entity ID | Type | Notes |
|-----------|------|-------|
| `sensor.gw3000a_soil_moisture_{N}` | sensor (%) | Per-channel volumetric moisture (1 %-quantized; can sit flat for hours on a healthy probe). Primary §3.2 input via the zone aggregate. |
| `sensor.gw3000a_soil_temperature_{N}` | sensor (°C) | Same RF packet; changes far more often than moisture → the primary liveness proxy. |
| `sensor.gw3000a_soil_electrical_conductivity_{N}` | sensor (µS/cm) | Same packet; second liveness proxy. (Live 2026-08-31: CH1 160 / CH2 160 / CH3 400 / CH4 180.) |
| `sensor.gw3000a_soil_battery_{N}` | sensor (V) | Probe battery (~1.5–1.6 V healthy). Flat value → not usable as a liveness signal. |

Channel↔label↔zone mapping and calibration set points live in
[`soil_calibration_log.md`](soil_calibration_log.md).
...
#### Seasonal Soil Moisture Thresholds (48 entities: 12 per zone) — REMOVED

> **REMOVED from the repo 2026-09-23** (ADR-021 Amendment A, §3.6 S2-rework).
> - They disappear from the Green at the next pull + restart. **Do not reference them.**
> - The spec below is kept only as history.
> - Their replacements are the per-zone moisture-guard helpers in the ADR-021 block above
>   (`zone_N_moisture_wet_veto` / `_dry_bump` / `_dry_bump_enabled` +
>   `moisture_guard_hysteresis`).
> - Their `initial:` values (35–80 %) are off-scale for this soil (real range ~12–34 % on
>   the factory scale).
...
| Date | Author | Change |
|------|--------|--------|
| 2026-09-23 | Claude | **§3.6 S2-rework built.** The moisture-guard helpers (`zone_N_moisture_wet_veto` / `_dry_bump` / `_dry_bump_enabled`, `moisture_guard_hysteresis`) are now **BUILT** in `packages/watering_state/moisture_guard.yaml`, with **0 = disabled** as the first-boot value. Added the hysteresis-held `binary_sensor.zone_{1-4}_moisture_wet` / `_dry`. The 48 seasonal `*_moisture_min` helpers are **REMOVED** from `zone_helpers.yaml`. The 80 weather thresholds are now RestoreEntity (no `initial:`). |
| 2026-09-23 | Claude | **ADR-021 Amendment A cascade.** ADR-021 block retitled "BUILT in repo / PLANNED" with a State column. **BUILT** (pushed; live after the next pull): `sensor.zone_{1-4}_soil_moisture` (now a guard input, WH52-only), `sensor.soil_moisture_pool`, `binary_sensor.wh52_soil_{1-4}_online` (3 h OR-of-3 liveness), `input_select.moisture_wireless_{1-4}_zone` (set CH4 → Blueberries after first pull), and the forecast sensors. **PLANNED:** per-zone `zone_N_moisture_wet_veto` / `_dry_bump` / `_dry_bump_enabled` + `moisture_guard_hysteresis`, plus `zone_N_current_status`. The 48 seasonal `*_moisture_min` helpers are marked **RETIRING**. SEN0600 is deferred indefinitely. |
