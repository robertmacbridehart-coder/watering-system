...
**Status:** ✅ PASSED LIVE 2026-08-25/26 (deployed on the Green; Model-2 supervised). **16/17
cases + both multipliers PASS**; only **DB-tag deferred** (needs a real end-to-end cycle → ESP32
must be online; it is intentionally offline while valves/pump are unwired). Method: zone 1, drive
`wp` by setting the fall thresholds against the fixed live weather + drive due/booster by
`interval_days` vs the fixed `days_since≈7`; the two SQL-dependent cases (①②) driven by an operator
`INSERT`/`UPDATE` on `zone_runs` (reversible); multipliers read from the real
`calculate_zone_runtime` response via Dev Tools → Actions (Return response).
...
**Cases (assert resulting `zone_N_program`):** *(live results 2026-08-25; days_since fixed at ≈7, so
the booster cases were driven at N=10 — the odd/even small-N examples are absorbed by day granularity
per the design, so N=10 is representative.)*
- [x] **Disabled:** `zone_N_enabled` OFF → `off`. **PASS** (enabled off → `off`).
- [x] **Not due:** enabled, days-since < N, weather normal → `off`. **PASS** (N=14 → `off`).
- [x] **Due, normal:** days-since ≥ N, weather normal → `normal`. **PASS** (N=3, W-normal → `normal`).
- [x] **Due, heavy (main):** days-since ≥ N, weather heavy → `heavy` (runtime 1.0×). **PASS** (N=3,
      W-heavy → `heavy`; `calculate_zone_runtime` → **1.0× / 30 min** via Dev Tools return-response).
- [x] **Rain gate on due day:** due but weather `off` → `off`. **PASS** (N=3, rain_off below live
      rain_72h → `off`).
- [x] **Odd N=3 booster** → `booster`. **PASS via N=10** (evening, W-heavy, pending → `booster`).
- [x] **Odd N=3 morning skip** → `off`. **PASS via N=10** (morning window, target evening → `off`).
- [x] **Even N=4 booster** → `booster`. **PASS via N=10** (parity absorbed by day granularity).
- [x] **Even N=4 single-window (morning only)** → `booster`. **PASS** (evening disabled, morning-only,
      target falls back to morning → `booster`).
- [x] **Booster retry after a miss** → `booster`. **PASS** — `last_booster` is `unknown` (DB confirms
      zero booster rows), so every booster case ran on the genuine pending/retry path.
- [x] **Booster already done (no re-fire):** `last_booster` set AFTER the last main → `off`. **PASS
      (2026-08-26)** — inserted a `heavy`+`0.5` row dated Aug-24 (after the Aug-18 main); `last_booster`
      → Aug-24, `zone_1_watering` stayed Aug-18 (booster excluded); at a valid evening booster slot
      (N=10, W-heavy, in-band) → `off` (pending=false, no re-fire). Test row deleted after.
- [x] **MAIN dose has priority over a pending booster** → `heavy` (MAIN), NOT `booster`. **PASS**
      (N=6, days_since 7 ≥ 6 → due → `heavy`; `due` is layered before `booster_slot`).
- [x] **Booster weather-cooled:** target evening re-evaluates to normal → `off`. **PASS** (N=10,
      W-normal, evening → `off`; booster fires only if still heavy).
- [x] **Overdue heavy:** days-since ≫ N → `heavy` (main). **PASS** (covered by N=3/N=6 due+heavy).
- [x] **Never-watered:** `zone_N_watering` = `unknown` → treated as due → waters. **PASS (2026-08-26)**
      — hid zone-1 history via `UPDATE zone_runs SET aborted=1 WHERE zone_id=1` (reversible; no delete),
      `zone_1_watering` → `unknown`, window_check (enabled, W-normal) → `normal` (days_since→9999 ⇒ due).
      Restored with `aborted=0` after.
- [x] **N==1 single-window heavy edge:** `calculate_zone_runtime` returns 1.5×. **PASS** — program
      resolves `heavy` (N=1, morning-only, due); `calculate_zone_runtime` → **1.5× / 45 min** via Dev
      Tools return-response (confirms the recorded `program_multiplier` = 1.5, not 1.0).
- [ ] **DB tag:** a booster run writes `heavy` + `program_multiplier=0.5`; `zone_N_watering` does NOT
      advance, `last_booster` DOES. **DEFERRED (ESP32 offline)** — needs a real end-to-end booster
      cycle (un-park + relay actuation + AppDaemon Event-3 write), but all relays currently read
      `unavailable` (ESP32 intentionally offline while valves/pump are unwired), so preflight would
      abort on comms and no row would be written. Do when the ESP32 is online. The AppDaemon write
      path was already verified live 2026-08-16 (Tests 10.x); the booster→`heavy`+`0.5` mapping is
      in `fire_zone_run_complete`. See START_HERE follow-up.

**DB validation (SQLite Web, read-only, 2026-08-25):** `zone_runs` holds 8 seed rows (cycle 1 normal
2026-08-16, cycle 2 heavy 2026-08-18), all `program_multiplier = NULL`. Confirms: `zone_N_watering` =
latest main dose (Aug-18); `last_booster` = `unknown` (no `heavy`+`0.5` rows); and the
`COALESCE(program_multiplier,1.0)=0.5` filter correctly treats the legacy NULL-multiplier heavy rows
as MAIN (not booster).

**Pass Criteria:** every case matches architecture.md §3.2/§3.3 (ADR-020, v1.9.0). **16/17 cases +
both multipliers + the DB sensor-queries verified live 2026-08-25/26**; only the **DB-tag** case is
outstanding, deferred until the ESP32 is online (a real end-to-end cycle). Offline Jinja sim (9
multiplier + 8 gate) had already passed 2026-08-23.
...
### Test 4.24: Weather-Primary Program Selection + Moisture Guard (window_check) — ADR-021 Amendment A
**Status:** ☐ NOT YET RUN — **respecified 2026-09-23** for ADR-021 Amendment A (weather-primary,
moisture as a guard). The earlier moisture-primary ladder spec was retired.
- **When:** run after §3.6 **S2-rework** (offline / Template editor + the recorded S3 rows),
  then again at **S4** (`state_window_check` rewire). Add copy-paste drive YAML when built.
- **Offline coverage (2026-09-23):** the logic of every case below is exercised by
  `tests/ha_templates/test_decision_tree.py` and `test_moisture_guard.py`, which render
  the real templates. The boxes stay unticked until the **live** run (HA wiring,
  `response_variable`, recorded rows).
- **Scope:** supersedes the *intensity* (`wp`) portion of the older weather-tree tests. The
  ADR-020 cadence layer (Test 4.22) is unchanged and still consumes `wp`.

**Prerequisites (at implementation):**
- `sensor.zone_N_soil_moisture` + `binary_sensor.wh52_soil_N_online` live.
- Guard helpers `zone_N_moisture_wet_veto` / `_dry_bump` / `_dry_bump_enabled` +
  `moisture_guard_hysteresis` set on the gateway-calibrated scale.
- `sensor.forecast_pop_today` / `_rain_today` / `forecast_temp_high_today` / `_tomorrow` live.

**How to drive it:** set moisture, rain_now/24h/72h, the forecast highs, forecast POP/volume and
season + thresholds, then call `script.compute_zone_programs` (S2/S3; return-response in Dev
Tools) or `script.state_window_check` (S4) and read each zone's program. As with Test 4.22, the
exact `wp` template can be validated offline / in the Template editor first.

**Cases (assert resulting `wp` → `zone_N_program`):**
- [ ] **2026-08-18 regression (headline):** cool, overcast, a local downpour DWD under-recorded,
      soil wet (moisture ≥ `wet_veto`) → **`off` via the wet veto**, even though the weather tree
      alone says water.
- [ ] **Weather tree branches** (primary): `weather_available` false → `normal`; `rain_now` →
      `off`; `rain_72h > rain_off` → `off`; `rain_24h > rain_light` → `light`;
      hot + dry → `heavy`; warm → `normal`; else `light`; temp unknown → `normal` (never heavy).
- [ ] **Window-aware de-lag:** the morning window reads `forecast_temp_high_today`, the evening
      reads `forecast_temp_high_tomorrow`. A lagging `temp_avg_high_3day` is ignored (no spurious
      `heavy`).
- [ ] **Wet veto:** moisture ≥ `wet_veto` → `off` (`skip_reason: moisture_wet`), applied last, and
      overrides everything including a dry bump.
- [ ] **Veto hysteresis:** once engaged, the veto holds until moisture < `wet_veto − hysteresis`. A
      ±1 dither at the threshold does not flip it.
- [ ] **Dry bump (enabled):** moisture ≤ `dry_bump` and `wp ≠ off` → +1 step, capped at `heavy`.
- [ ] **Dry bump (disabled, default):** same inputs → no bump.
- [ ] **Stale / unknown moisture:** no fresh sensor (liveness gate off) → the guard is inactive,
      the weather tree decides alone, never fail-`heavy`.
- [ ] **Forecast downgrade fires:** `forecast_pop_today > 80` AND `forecast_rain_today ≥ 5` →
      ≤ 2 steps: `heavy → light` (NOT off), `normal → off`, `light → off`. It runs after the dry
      bump and before the veto.
- [ ] **Forecast downgrade gated off:** POP ≤ 80 OR volume < 5 → no downgrade.
- [ ] **Cadence still works on the new `wp`:** feed the resulting `wp` through ADR-020. The `due`
      main dose, booster slot and rain-`off` retry must behave exactly as in Test 4.22.
- [ ] **Threshold RestoreEntity:** tune a guard/weather threshold, restart HA, and confirm the tuned
      value persists (pure RestoreEntity, no `initial:`).
- [ ] **Decision recording — run (ADR-018):** a watered zone stamps the inputs (moisture % +
      sensor count, rain, the forecast high + horizon, POP/volume, thresholds, guard state,
      branch) into `zone_runs.decision_criteria` and the matching `zone_decisions` row.
- [ ] **Decision recording — skip (ADR-018):** a skipped/parked window still writes a
      `zone_decisions` row (always-on logger) with `would_water=0` and the branch /
      `skip_reason` (disabled / not_due / raining / rain_72h / forecast_downgrade /
      moisture_wet).

- [x] **S4 wiring (live, parked):** `script.state_window_check` applies exactly the programs
      `compute_zone_programs` returns for the active window, logs the `Programs (<window>): …`
      breadcrumb, and does **not** advance the state (A1; parked). Drive below.
      ✅ **PASSED LIVE 2026-09-23** (Claude via hass-mcp, operator-authorised, parked):
      - **A (all zones disabled):** the trace ran window_check → compute_zone_programs → 4×
        resolve_program_multiplier → 1× log_system_event. All four programs went `normal` →
        `off`; the state stayed `manual_override`.
      - **B (zone 1 enabled temporarily):** predicted `normal` from the inputs (fall; warm
        branch: today's high 18.3 ≥ 18, < 25; 72 h 3.1 mm; POP 1 %; due, N = 1). The live
        result was `zone_1_program = normal`. The fail-safe would have been `off`, so this
        proves the response_variable round-trip, extraction and validation. The state stayed
        `manual_override`; zone 1 was disabled again.
      - **Breadcrumb verified** in `watering_ops.system_events`: rows 222 (A, all
        `off (disabled)`) and 223 (B, `Z1 normal · Z2 off (disabled) · …`), text exactly as
        designed.
      - **C (un-parked, operator-driven after Claude's button press was classifier-blocked):**
        override off → idle → `start_watering_cycle_now`. The scheduler's manual branch set
        `active_watering_window = evening` and `active_trigger_type = manual`, then
        `window_check`. The dispatcher ran the new check: `Programs (evening): Z1 off
        (disabled) · … Z4 off (disabled)`, then `All zones resolved to 'off' - nothing due.
        Returning to idle.` → `idle` in ~180 ms.
        - Preflight was never reached (last run 2026-08-18); no error event.
        - A second press at 11:15:09 repeated the cycle identically (~90 ms).
        - Re-parked afterwards.
- [ ] **S4 fail-safe:** a routine error / malformed result → all zones `off`, `error` log,
      → idle. Offline only (`tests/ha_templates/test_s4_window_check.py`); cannot be forced
      live without editing code.

**S4 live drive (Dev Tools, parked; supervised).** Only config helpers are written
(`active_watering_window`, `zone_N_program`); A1 blocks any state change while
`manual_override` is the state.

1. **Observation template** (Dev Tools → Template; keep it open):
   ```jinja
   {% for z in [1, 2, 3, 4] %}zone_{{ z }}_program: {{ states('input_select.zone_' ~ z ~ '_program') }} (enabled {{ states('input_boolean.zone_' ~ z ~ '_enabled') }})
   {% endfor %}state: {{ states('input_select.watering_system_state') }}
   override: {{ states('input_boolean.manual_override_active') }}
   window: {{ states('input_select.active_watering_window') }}
   relays on: {{ states.switch | selectattr('entity_id', 'search', 'watering_system_relay') | selectattr('state', 'eq', 'on') | map(attribute='entity_id') | list }}
   ```
   Precondition: `state: manual_override`, `override: on`, `relays on: []`.
2. **Set the window** (Dev Tools → Actions):
   ```yaml
   action: input_select.select_option
   target:
     entity_id: input_select.active_watering_window
   data:
     option: morning
   ```
3. **Expected programs** (Actions, tick "return response"):
   ```yaml
   action: script.compute_zone_programs
   data:
     window: morning
   ```
   Note each `zone_decisions[].computed_program` (by `zone_id`).
4. **Arm one event:** Dev Tools → Events → listen to `watering_system_event` (fresh tab).
5. **Apply:**
   ```yaml
   action: script.state_window_check
   ```
6. **Pass:**
   - the template's four `zone_N_program` match step 3;
   - `state` is still `manual_override`, `relays on: []`;
   - one `watering_system_event` with `event_type: state_window_check`, severity `info`,
     message `Programs (morning): Z1 … · Z2 … · Z3 … · Z4 …`;
   - no `error` event.
7. **Optional non-off case:** turn `input_boolean.zone_1_enabled` on, repeat steps 3–6
   (zone 1 should get its due program, e.g. `normal`), then turn it **off** again.

**Pass Criteria:**
- Every case matches architecture.md §3.2 (v1.10.1, ADR-021 Amendment A; S4 applies it).
- Weather is primary, and moisture only ever vetoes or bumps.
- With stale moisture the weather tree runs alone; nothing fails heavy.
- The forecast downgrade is capped and floored.
- Decisions are recorded (runs + skips) and cross-referable to `zone_runs`.
...
### Test 10.10: S5 Decision Recording + Weather Export (live)

**Test ID:** 10.10
**Component:** `db_schema_init.py` (column migration), `db_writer.py` (Event 7 buffer),
`db_export.py` (weather tables); HA: `state_window_check` Event 7, Event 3 `season`, Event 1 sources
**Objective:** Verify the §3.6 S5 changes on the Green: the live `zone_runs` table gains `season` +
`decision_criteria` without losing rows, and the seasonal export includes the weather DB.
**Phase:** 3.6 S5 (ADR-018)
**Status:** ✅ **PASS (2026-09-23)** — all four live checks. The real-cycle row check stays deferred
until the ESP32 is online.
**Last Run:** 2026-09-23 (after the `abb1c62` pull + AppDaemon add-on restart)
**Offline coverage (all passing):** `watering_db/tests/test_db_schema_init.py` (11: fresh / pre-S5 /
re-run / CHECK), `test_db_writer.py` (39 incl. 15 S5: buffer, attach, stale, missing, bad season),
`test_db_export.py` (22 incl. weather tables + missing weather DB), and
`tests/ha_templates/test_s5_decision_recording.py` (14: real routine output → Event 7 template →
REAL `DbWriter` → `zone_runs` row; Event 3 `season`; Event 1 sources).

**Test Steps:**
1. **AppDaemon log after the pull** (it reloads changed apps). Expect, from the first start after the
   pull:
   - `watering_ops schema verified/created at /homeassistant/watering_ops.db (4 tables present;
     columns added: zone_runs.season, zone_runs.decision_criteria)`;
   - `DbWriter ready; listening for 'watering_decision_applied', …`;
   - `DbSeasonalExport ready; …`.
   If the schema line is missing, or says `columns added: none` before step 2 shows the columns,
   restart the AppDaemon add-on once and re-check.
2. **Columns** (SQLite Web on `watering_ops.db`):
   ```sql
   SELECT name, type FROM pragma_table_info('zone_runs');
   SELECT COUNT(*) AS rows, COUNT(season) AS with_season, COUNT(decision_criteria) AS with_criteria
   FROM zone_runs;
   ```
3. **Export** (Dev Tools → Events → fire `watering_seasonal_export`, data `year: 2026`). It rewrites
   the 2026 CSVs in `/homeassistant/watering_exports/` with current data (a superset of the
   2026-08-16 files).
4. **Audit row** (SQLite Web on `watering_ops.db`):
   ```sql
   SELECT event_id, timestamp, severity, notes FROM system_events
   WHERE event_type = 'seasonal_export' ORDER BY event_id DESC LIMIT 1;
   ```

**Expected Results:**
- [x] Step 1 log lines present; the columns-added line names both columns. ✅ AppDaemon's
      automatic reload of the changed apps at 21:56:25 Berlin logged `columns added:
      zone_runs.season, zone_runs.decision_criteria`. The add-on restart at 21:58:02 logged
      `columns added: none` (idempotent). `DbWriter ready; listening for
      'watering_decision_applied', …` and `DbSeasonalExport ready` were both logged.
- [x] Step 2: `season` + `decision_criteria` listed last; every existing row counted, with
      `with_season` = `with_criteria` = 0 (pre-S5 rows keep NULL; no row lost). ✅ Both columns
      are last (TEXT); `8 | 0 | 0`.
- [x] Step 3: `weather_snapshots_2026.csv` + `zone_decisions_2026.csv` created alongside the four
      ops CSVs; `zone_decisions` = 4 × the snapshot rows. ✅ 3 snapshots / 12 decisions.
- [x] Step 4: severity `info`; notes include `weather_snapshots=` and `zone_decisions=` counts.
      ✅ `system_events` 230: `Exported year 2026: watering_cycles=2, zone_runs=8,
      fertigation_doses=0, system_events=227, weather_snapshots=3, zone_decisions=12`.
- [ ] **Deferred (needs the ESP32 online; pair with Test 4.22's DB-tag cycle):** a real cycle writes
      `zone_runs.season` + `decision_criteria` (JSON of that zone's applied decision), and its
      `watering_cycles` row stores `sensor.rain_24h` / `rain_72h` and the window's forecast high.

**Pass Criteria:** the migration adds both columns in place with no data loss, and one export trigger
produces all six CSVs with an `info` audit row.
...
- ✅ 10.10 S5 decision recording + weather export (2026-09-23: migration, columns, export; real-cycle row deferred)
...
- **2026-09-23:** **Test 10.10 PASSED live** after the `abb1c62` pull: the migration added both
  `zone_runs` columns in place (8 rows kept, NULL) and was idempotent on the add-on restart; the Event 7
  listener is live; the export wrote 3 weather snapshots + 12 decisions with an `info` audit row (230).
  The real-cycle row check is deferred until the ESP32 is online.
- **2026-09-23:** **Test 10.10 added (§3.6 S5):** live checks for the `zone_runs` column migration,
  the Event 7 writer, and the weather tables in the seasonal export (copy-paste SQL). The real-cycle
  row check is deferred until the ESP32 is online.
- **2026-09-23:** **Test 4.24 S4 wiring PASSED LIVE**: the parked all-off case, the zone-1
  `normal` case, the breadcrumb text (`system_events` 222/223), and the un-parked full path
  (scheduler manual → dispatcher → window_check → all off → idle; no preflight).
- **2026-09-23:** **Test 4.24: S4 cases + live drive added.** Two new boxes (S4 wiring live;
  S4 fail-safe offline-only) and a copy-paste Dev-Tools drive for the parked live pass
  (observation template, set window, routine return-response, `state_window_check`, pass
  criteria, optional zone-1 enable case).
- **2026-09-23:** **Test 4.24 respecified for ADR-021 Amendment A** (weather-primary, moisture
  as guard). The moisture-ladder cases (ladder bands, ±1 modifiers, no-escalate-from-moist) are
  retired. New cases:
  - wet veto, applied last and hysteresis-held;
  - dry bump, enabled and disabled;
  - stale / unknown moisture → weather alone;
  - window-aware de-lag (morning today / evening tomorrow);
  - the full weather-tree branch set.

  The 2026-08-18 regression now resolves `off` **via the wet veto**. The run point moves to
  after §3.6 S2-rework and again at S4. Not run.
