...
the system_events audit row. Also builds a weather DB from docs/weather_schema.sql
and asserts the weather_snapshots / zone_decisions export (year filter on the
ISO observed_at, incl. a legacy +02:00 row; decisions follow their snapshots;
read-only), and that a MISSING weather DB skips only those two files and marks
the audit row `warning`. Exits non-zero on any failure.
...
WEATHER_SCHEMA = os.path.join(REPO, "docs", "weather_schema.sql")
...
def _build_weather_db(path):
    with open(WEATHER_SCHEMA, "r", encoding="utf-8") as fh:
        sql = fh.read()
    conn = sqlite3.connect(path)
    conn.executescript(sql)
    snaps = [
        (1, "2026-09-23T01:20:54.519824+02:00", "morning"),  # legacy local-offset row
        (2, "2026-09-23T16:00:00.100000+00:00", "evening"),  # UTC row
        (3, "2025-10-01T04:00:00.000000+00:00", "morning"),  # other year -> excluded
    ]
    for sid, obs, win in snaps:
        conn.execute(
            "INSERT INTO weather_snapshots (snapshot_id, observed_at, window_name) "
            "VALUES (?, ?, ?)", (sid, obs, win))
        for zone in (1, 2):
            conn.execute(
                "INSERT INTO zone_decisions (snapshot_id, zone_id, season, "
                "computed_program, would_water, decision_criteria) "
                "VALUES (?, ?, 'fall', 'off', 0, '{}')", (sid, zone))
    conn.commit()
    conn.close()
...
def main():
    ...
    weather_path = os.path.join(tmp, "watering_weather.db")
    export_dir = os.path.join(tmp, "exports")
    _build_db(db_path)
    _build_weather_db(weather_path)
    ...
    app.weather_db_path = weather_path
    ...
    if audit:
        ...
        check("weather_snapshots=2" in audit[0][1] and "zone_decisions=4" in audit[0][1],
              "audit notes carry the weather counts")
        print("  audit notes:", audit[0][1])

    # ---- weather DB export ---------------------------------------------------
    wfile = os.path.join(export_dir, "weather_snapshots_2026.csv")
    dfile = os.path.join(export_dir, "zone_decisions_2026.csv")
    check(os.path.exists(wfile) and os.path.exists(dfile), "weather CSVs created")
    with open(wfile, newline="", encoding="utf-8") as fh:
        wrows = list(csv.reader(fh))
    check(wrows[0][0] == "snapshot_id", "weather_snapshots header present")
    check(sorted(r[0] for r in wrows[1:]) == ["1", "2"],
          "2026 snapshots incl. the legacy +02:00 row; 2025 excluded")
    with open(dfile, newline="", encoding="utf-8") as fh:
        drows = list(csv.reader(fh))
    snap_col = drows[0].index("snapshot_id")
    check(len(drows) == 5 and {r[snap_col] for r in drows[1:]} == {"1", "2"},
          "zone_decisions follow their 2026 snapshots (4 rows)")
    wconn = sqlite3.connect(weather_path)
    wcount = wconn.execute("SELECT COUNT(*) FROM weather_snapshots").fetchone()[0]
    wconn.close()
    check(wcount == 3, "weather DB untouched (export only, nothing deleted)")

    # ---- missing weather DB: ops export still succeeds, audit = warning ------
    export_dir2 = os.path.join(tmp, "exports2")
    app.export_dir = export_dir2
    app.weather_db_path = os.path.join(tmp, "does_not_exist.db")
    app.run_export("watering_seasonal_export", {"year": 2026}, {})
    check(os.path.exists(os.path.join(export_dir2, "zone_runs_2026.csv")),
          "missing weather DB: ops CSVs still written")
    check(not os.path.exists(os.path.join(export_dir2, "weather_snapshots_2026.csv")),
          "missing weather DB: weather CSVs skipped")
    check(not os.path.exists(os.path.join(tmp, "does_not_exist.db")),
          "missing weather DB not created by the read-only open")
    conn = sqlite3.connect(db_path)
    last = conn.execute(
        "SELECT severity, notes FROM system_events WHERE event_type = 'seasonal_export' "
        "ORDER BY event_id DESC LIMIT 1").fetchone()
    conn.close()
    check(last is not None and last[0] == "warning" and "not found" in last[1],
          f"missing weather DB: audit row severity warning (got {last})")
