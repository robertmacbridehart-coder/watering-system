...
HA_AD_WEATHER="$HA_AD_ROOT/watering_weather"
...
SRC_WEATHER_SCHEMA="$SRCDIR/docs/weather_schema.sql"
if [ -d "$SRC_AD_ROOT" ]; then
  if [ "$DRY_RUN" = "1" ]; then
    ...
    # watering_weather: refresh the canonical weather schema alongside the app
    # (same anti-drift pattern -- deployed from the single source docs/weather_schema.sql).
    if [ -d "$HA_AD_WEATHER" ]; then
      if [ -f "$SRC_WEATHER_SCHEMA" ]; then
        echo "[pull] docs/weather_schema.sql -> $HA_AD_WEATHER/weather_schema.sql"
        cp -f "$SRC_WEATHER_SCHEMA" "$HA_AD_WEATHER/weather_schema.sql"
      else
        echo "[pull] WARN: missing in repo: docs/weather_schema.sql (weather schema NOT refreshed)"
      fi
    fi
  fi
else
